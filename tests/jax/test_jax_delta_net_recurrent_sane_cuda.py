...
from rwkv_ops.delta_net_recurrent_sane.jax_cuda_kernel.delta_net_recurrent_sane_jax import (
    delta_net_recurrent_sane as dn_cuda_sane,
    delta_net_recurrent_sane_inference as dn_cuda_sane_inference,
    delta_net_recurrent_sane_single_step as dn_cuda_sane_single_step,
)
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_cuda_sane_forward_matches_native(
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_cuda_sane_backward_matches_native(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_cuda_sane_bfloat16_io(delta_net_sane_inputs, dn_sane_jax_cuda_device):
...
@pytest.mark.jax
def test_dn_cuda_sane_no_mask_warning_and_none_state(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
...
@pytest.mark.jax
def test_dn_cuda_sane_all_one_mask_equals_no_mask(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
    ...
    with warnings.catch_warnings(record=True):
        warnings.simplefilter("always")
        y_no_mask, s_no_mask = dn_cuda_sane(
            q, k, v, beta, tau, mask=None, initial_state=h0, output_final_state=True
        )

    y_all_one, s_all_one = dn_cuda_sane(
        q,
        k,
        v,
        beta,
...
@pytest.mark.jax
def test_dn_cuda_sane_all_zero_mask_matches_non_sane(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
...
@pytest.mark.jax
def test_dn_cuda_sane_inference_matches_native(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
    ...
    out_cuda, state_cuda = dn_cuda_sane_inference(
        q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
    )
...
@pytest.mark.jax
def test_dn_cuda_sane_inference_arbitrary_length(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
    """JAX CUDA SANE 推理算子支持 T 不被 16 整除的任意长度。"""
    T = 34
    device = dn_sane_jax_cuda_device
    q = _to_jax_tensor(delta_net_sane_inputs["q"][:, :T], device, jnp.bfloat16)
    k = _to_jax_tensor(delta_net_sane_inputs["k"][:, :T], device, jnp.bfloat16)
    v = _to_jax_tensor(delta_net_sane_inputs["v"][:, :T], device, jnp.bfloat16)
    beta = _to_jax_tensor(delta_net_sane_inputs["beta"][:, :T], device, jnp.float32)
    tau = _to_jax_tensor(
        delta_net_sane_inputs["tau"][:, : (T // 16), :], device, jnp.float32
    )
    mask = _to_jax_tensor(
        delta_net_sane_inputs["mask"][:, : (T // 16)], device, jnp.float32
    )
    h0 = _to_jax_tensor(delta_net_sane_inputs["h0"], device, jnp.float32)
...
@pytest.mark.jax
def test_dn_cuda_sane_single_step(delta_net_sane_inputs, dn_sane_jax_cuda_device):
    """JAX CUDA SANE 单步 RNN：do_sane=1 对 native SANE，do_sane=0 对原 GDN。"""
    device = dn_sane_jax_cuda_device
    q = _to_jax_tensor(delta_net_sane_inputs["q"][:, 0], device, jnp.bfloat16)
    k = _to_jax_tensor(delta_net_sane_inputs["k"][:, 0], device, jnp.bfloat16)
    v = _to_jax_tensor(delta_net_sane_inputs["v"][:, 0], device, jnp.bfloat16)
    beta = _to_jax_tensor(delta_net_sane_inputs["beta"][:, 0], device, jnp.float32)
    tau = _to_jax_tensor(delta_net_sane_inputs["tau"][:, 0, :], device, jnp.float32)
    h0 = _to_jax_tensor(delta_net_sane_inputs["h0"], device, jnp.float32)
    B = delta_net_sane_inputs["q"].shape[0]

    do_sane = jnp.full((B,), 1.0, dtype=jnp.float32)
    out_cuda, state_cuda = dn_cuda_sane_single_step(
...
@pytest.mark.jax
def test_dn_cuda_sane_head_first(delta_net_sane_inputs, dn_sane_jax_cuda_device):
    """JAX CUDA SANE 训练算子 head_first 布局与默认布局结果一致。"""
    q, k, v, beta, tau, mask, h0 = _prepare_sane_inputs(
        delta_net_sane_inputs, dn_sane_jax_cuda_device, dtype="bfloat16"
    )

    out_ref, state_ref = dn_cuda_sane(
        q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
    )
    out_hf, state_hf = dn_cuda_sane(
...
@pytest.mark.jax
def test_dn_cuda_sane_rejects_arbitrary_length(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
    """JAX CUDA SANE 训练算子拒绝不被 chunk_size 整除的序列长度。"""
    device = dn_sane_jax_cuda_device
    q = _to_jax_tensor(delta_net_sane_inputs["q"][:, :120], device)
    k = _to_jax_tensor(delta_net_sane_inputs["k"][:, :120], device)
    v = _to_jax_tensor(delta_net_sane_inputs["v"][:, :120], device)
    beta = _to_jax_tensor(delta_net_sane_inputs["beta"][:, :120], device)
    tau = _to_jax_tensor(delta_net_sane_inputs["tau"], device)

    with pytest.raises(ValueError):
        dn_cuda_sane(q, k, v, beta, tau, output_final_state=False)


@pytest.mark.jax
@pytest.mark.slow
def test_dn_cuda_sane_forward_chunk_size(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
    ...
    tau, mask = _make_tau_mask_for_chunk_size(
        delta_net_sane_inputs, chunk_size, dn_sane_jax_cuda_device
    )
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_cuda_sane_backward_chunk_size(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
...
@pytest.mark.jax
def test_dn_cuda_sane_inference_chunk_size(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
...
@pytest.mark.jax
def test_dn_cuda_sane_irregular_padding(delta_net_sane_inputs, dn_sane_jax_cuda_device):
    ...
    q_np = delta_net_sane_inputs["q"][:, :pad_len].copy()
    k_np = delta_net_sane_inputs["k"][:, :pad_len].copy()
    v_np = delta_net_sane_inputs["v"][:, :pad_len].copy()
    beta_np = delta_net_sane_inputs["beta"][:, :pad_len].copy()
    for arr in (q_np, k_np, v_np, beta_np):
        arr[:, actual_len:] = 0.0

    tau_np = delta_net_sane_inputs["tau"][:, : pad_len // 16].copy()
    ...
    _, state_ref = dn_native_recurrent(
        q[:, 32:actual_len],
        k[:, 32:actual_len],
        v[:, 32:actual_len],
        beta[:, 32:actual_len],
...
@pytest.mark.jax
def test_dn_cuda_sane_sharding_structure(
    delta_net_sane_inputs, dn_sane_jax_cuda_device
):
    ...
    out_jit, state_jit = jax.jit(
        lambda q, k, v, beta, tau, mask, h0: dn_cuda_sane(
            q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
        )
    )(qs, ks, vs, bs, ts, ms, hs)
    out_ref, state_ref = dn_cuda_sane(
        q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
    )
