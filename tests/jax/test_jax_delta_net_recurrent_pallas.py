"""DeltaNet recurrent Pallas kernel 的 JAX 后端测试。"""
...
from rwkv_ops.delta_net_recurrent.jax_pallas_kernel import (
    delta_net_recurrent as dn_pallas_recurrent,
    delta_net_recurrent_inference as dn_pallas_inference,
    delta_net_recurrent_single_step as dn_pallas_single_step,
)
...
def dn_jax_device():
    ...
    if jax.devices()[0].platform not in ("gpu", "tpu"):
        pytest.skip("DeltaNet recurrent Pallas kernel requires JAX GPU/TPU.")
...
@pytest.mark.jax
@pytest.mark.slow
def test_delta_net_pallas_recurrent_matches_native(delta_net_inputs, dn_jax_device):
    ...
    out_pallas, state_pallas = dn_pallas_recurrent(
...
@pytest.mark.jax
def test_delta_net_pallas_inference_matches_native(delta_net_inputs, dn_jax_device):
    ...
    out_pallas, state_pallas = dn_pallas_inference(
        q, k, v, beta, initial_state=h0, output_final_state=True
    )
...
@pytest.mark.jax
def test_delta_net_pallas_single_step_matches_native(delta_net_inputs, dn_jax_device):
    ...
    out_pallas, state_pallas = dn_pallas_single_step(
        q, k, v, beta, initial_state=h0, output_final_state=True
    )
...
@pytest.mark.jax
@pytest.mark.slow
def test_delta_net_pallas_recurrent_backward(delta_net_inputs, dn_jax_device):
    ...
    _, grads_pallas = _dn_value_and_grad(dn_pallas_recurrent, q, k, v, beta, h0)
...
@pytest.mark.jax
@pytest.mark.slow
def test_delta_net_pallas_recurrent_chunk_size(delta_net_inputs, dn_jax_device):
...
@pytest.mark.jax
@pytest.mark.slow
def test_delta_net_pallas_recurrent_backward_chunk_size(
    delta_net_inputs, dn_jax_device
):
    """JAX Pallas recurrent 训练算子非默认 chunk_size 反向梯度与 native 对齐。"""
    chunk_size = 8
    q = _to_jax_tensor(delta_net_inputs["q"], dn_jax_device)
    k = _to_jax_tensor(delta_net_inputs["k"], dn_jax_device)
    v = _to_jax_tensor(delta_net_inputs["v"], dn_jax_device)
    ...
    _, grads_pallas = jax.value_and_grad(
        lambda q, k, v, beta, h0: loss_fn(dn_pallas_recurrent, q, k, v, beta, h0),
        argnums=(0, 1, 2, 3, 4),
    )(q, k, v, beta, h0)
...
@pytest.mark.jax
@pytest.mark.slow
def test_delta_net_pallas_recurrent_bf16(delta_net_inputs, dn_jax_device):
    """JAX Pallas recurrent 训练算子 bf16 前向与 native 参考对齐。"""
    ...
    assert_allclose_with_stats(
        out_ref,
        out_pallas,
        "pallas recurrent bf16 vs native output",
        atol=1e-2,
        rtol=1e-2,
    )
    assert_allclose_with_stats(
        state_ref,
        state_pallas,
        "pallas recurrent bf16 vs native state",
        atol=1e-2,
        rtol=1e-2,
    )
...
@pytest.mark.jax
def test_delta_net_pallas_inference_bf16(delta_net_inputs, dn_jax_device):
    """JAX Pallas recurrent 推理算子 bf16 前向与 native 参考对齐。"""
    q = _to_jax_tensor(delta_net_inputs["q"], dn_jax_device, jnp.bfloat16)
    k = _to_jax_tensor(delta_net_inputs["k"], dn_jax_device, jnp.bfloat16)
    v = _to_jax_tensor(delta_net_inputs["v"], dn_jax_device, jnp.bfloat16)
    beta = _to_jax_tensor(delta_net_inputs["beta"], dn_jax_device, jnp.float32)
    ...
    assert_allclose_with_stats(
        out_ref,
        out_pallas,
        "pallas inference bf16 vs native output",
        atol=1e-2,
        rtol=1e-2,
    )
    assert_allclose_with_stats(
        state_ref,
        state_pallas,
        "pallas inference bf16 vs native state",
        atol=1e-2,
        rtol=1e-2,
    )
...
@pytest.mark.jax
def test_delta_net_pallas_single_step_bf16(delta_net_inputs, dn_jax_device):
    """JAX Pallas recurrent 单步 RNN 算子 bf16 前向与 native 参考对齐。"""
    ...
    assert_allclose_with_stats(
        out_ref,
        out_pallas,
        "pallas single step bf16 vs native output",
        atol=1e-2,
        rtol=1e-2,
    )
    assert_allclose_with_stats(
        state_ref,
        state_pallas,
        "pallas single step bf16 vs native state",
        atol=1e-2,
        rtol=1e-2,
    )
...
@pytest.mark.jax
def test_delta_net_pallas_recurrent_no_initial_state(delta_net_inputs, dn_jax_device):
    """JAX Pallas recurrent 训练算子 initial_state=None 与 native 参考对齐。"""
    q = _to_jax_tensor(delta_net_inputs["q"], dn_jax_device)
    k = _to_jax_tensor(delta_net_inputs["k"], dn_jax_device)
    v = _to_jax_tensor(delta_net_inputs["v"], dn_jax_device)
    beta = _to_jax_tensor(delta_net_inputs["beta"], dn_jax_device)
    ...
    assert_allclose_with_stats(
        out_ref,
        out_pallas,
        "pallas recurrent no initial_state vs native output",
        atol=1e-4,
        rtol=1e-3,
    )
    assert_allclose_with_stats(
        state_ref,
        state_pallas,
        "pallas recurrent no initial_state vs native state",
        atol=1e-4,
        rtol=1e-3,
    )
...
@pytest.mark.jax
def test_delta_net_pallas_recurrent_broadcast_initial_state(
    delta_net_inputs, dn_jax_device
):
    """JAX Pallas recurrent 训练算子 [1, H, K, V] 广播 initial_state 与 native 对齐。"""
    q = _to_jax_tensor(delta_net_inputs["q"], dn_jax_device)
    k = _to_jax_tensor(delta_net_inputs["k"], dn_jax_device)
    v = _to_jax_tensor(delta_net_inputs["v"], dn_jax_device)
    beta = _to_jax_tensor(delta_net_inputs["beta"], dn_jax_device)
    ...
    assert_allclose_with_stats(
        out_ref,
        out_pallas,
        "pallas recurrent broadcast initial_state vs native output",
        atol=1e-4,
        rtol=1e-3,
    )
    assert_allclose_with_stats(
        state_ref,
        state_pallas,
        "pallas recurrent broadcast initial_state vs native state",
        atol=1e-4,
        rtol=1e-3,
    )
