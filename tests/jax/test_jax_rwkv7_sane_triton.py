"""RWKV-7 State Anomaly Neutralization JAX Triton kernel 数值测试。"""
...
def test_rwkv7_sane_triton_forward_state(
    rwkv7_sane_jax_triton_op, rwkv7_sane_native_op, rwkv7_sane_inputs, head_first
):
    ...
    y_c, s_c = rwkv7_sane_jax_triton_op(
        r=r_c,
        w=w_c,
        k=k_c,
        v=v_c,
...
def test_rwkv7_sane_triton_backward(
    ...
    triton_grads = jax.grad(
        lambda *p: loss(rwkv7_sane_jax_triton_op, p), argnums=range(9)
    )(w, r, k, v, a, b, tau, mask, h0)
...
def test_rwkv7_sane_triton_no_mask_backward(
    ...
    triton_grads = jax.grad(
        lambda *p: loss(rwkv7_sane_jax_triton_op, p), argnums=range(8)
    )(w, r, k, v, a, b, tau, h0)
...
def test_rwkv7_sane_triton_no_mask_y_matches_all_one(
    rwkv7_sane_jax_triton_op, rwkv7_sane_inputs
):
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        y_no_mask, s_no_mask = rwkv7_sane_jax_triton_op(
            r=r,
            w=w,
            k=k,
            v=v,
    ...
    y_all_one, s_all_one = rwkv7_sane_jax_triton_op(
        r=r,
        w=w,
        k=k,
        v=v,
...
@pytest.mark.jax
def test_rwkv7_sane_triton_head_sharding(rwkv7_sane_jax_triton_op, rwkv7_sane_inputs):
    """验证 head 维度可以沿 'h' 轴分片（TP）。单 GPU 下用 1-device mesh 模拟。"""
    devices = jax.devices()
    mesh = Mesh(devices, ("h",))

    # triton 内部是 head-first [B, N, T, H]
    ...
    def run(r, w, k, v, a, b, tau, mask, h0):
        return rwkv7_sane_jax_triton_op(
            r=r,
            w=w,
            k=k,
            v=v,
