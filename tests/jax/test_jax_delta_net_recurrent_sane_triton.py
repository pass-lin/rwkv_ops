"""Gated DeltaNet recurrent SANE Triton kernel 的 JAX 后端测试。"""
...
from rwkv_ops.delta_net_recurrent_sane.jax_triton_kernel import (
    delta_net_recurrent_sane as dn_triton_recurrent,
    delta_net_recurrent_sane_inference as dn_triton_inference,
    delta_net_recurrent_sane_single_step as dn_triton_single_step,
)
...
def dn_sane_jax_triton_device():
    """JAX-Triton SANE kernel 需要 JAX GPU，否则跳过整个文件。"""
    if jax.devices()[0].platform != "gpu":
        pytest.skip("Gated DeltaNet recurrent SANE JAX-Triton kernel requires JAX GPU.")
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_forward_matches_native(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    """JAX-Triton SANE 训练算子前向/最终 state 与 native Keras 参考对齐。"""
    q, k, v, beta, tau, mask, h0 = _prepare_sane_inputs(
        delta_net_sane_inputs, dn_sane_jax_triton_device, dtype="bfloat16"
    )

    out_ref, state_ref = gdn_native_sane(
        q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
    )
    out_triton, state_triton = dn_triton_recurrent(
        q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
    )
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_backward_matches_native(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    """JAX-Triton SANE 训练算子反向梯度（含 dtau）与 native 参考对齐。"""
    q, k, v, beta, tau, mask, h0 = _prepare_sane_inputs(
        delta_net_sane_inputs, dn_sane_jax_triton_device, dtype="bfloat16"
    )
    ...
    for name, gr, gt in zip(names, ref_grads, triton_grads):
        assert_allclose_with_stats(
            gr, gt, f"grad_{name} triton vs native", atol=7e-3, rtol=1e-3
        )


@pytest.mark.jax
def test_dn_triton_sane_no_mask_warning_and_none_state(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        out, final_state = dn_triton_recurrent(
...
@pytest.mark.jax
def test_dn_triton_sane_all_one_mask_equals_no_mask(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    ...
    with warnings.catch_warnings(record=True):
        warnings.simplefilter("always")
        y_no_mask, s_no_mask = dn_triton_recurrent(
            q, k, v, beta, tau, mask=None, initial_state=h0, output_final_state=True
        )

    y_all_one, s_all_one = dn_triton_recurrent(
        q,
        k,
        v,
        beta,
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_all_zero_mask_matches_non_sane(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    ...
    out_triton, state_triton = dn_triton_recurrent(
        q, k, v, beta, tau, mask=zero_mask, initial_state=h0, output_final_state=True
    )
...
@pytest.mark.jax
def test_dn_triton_sane_inference_matches_native(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    """JAX-Triton SANE 推理算子前向/最终 state 与 native 参考对齐。"""
    q, k, v, beta, tau, mask, h0 = _prepare_sane_inputs(
        delta_net_sane_inputs, dn_sane_jax_triton_device, dtype="bfloat16"
    )

    out_triton, state_triton = dn_triton_inference(
        q, k, v, beta, tau, mask=mask, initial_state=h0, output_final_state=True
    )
...
@pytest.mark.jax
def test_dn_triton_sane_inference_arbitrary_length(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    """JAX-Triton SANE 推理算子支持 T 不被 16 整除的任意长度。"""
    T = 34
    q = _to_jax_tensor(
        delta_net_sane_inputs["q"][:, :T], dn_sane_jax_triton_device, jnp.bfloat16
    )
    k = _to_jax_tensor(
        delta_net_sane_inputs["k"][:, :T], dn_sane_jax_triton_device, jnp.bfloat16
    )
    v = _to_jax_tensor(
        delta_net_sane_inputs["v"][:, :T], dn_sane_jax_triton_device, jnp.bfloat16
    )
    beta = _to_jax_tensor(
        delta_net_sane_inputs["beta"][:, :T], dn_sane_jax_triton_device, jnp.float32
    )
    tau = _to_jax_tensor(
        delta_net_sane_inputs["tau"][:, : (T // 16), :],
        dn_sane_jax_triton_device,
        jnp.float32,
    )
    mask = _to_jax_tensor(
        delta_net_sane_inputs["mask"][:, : (T // 16)],
        dn_sane_jax_triton_device,
        jnp.float32,
    )
    h0 = _to_jax_tensor(
        delta_net_sane_inputs["h0"], dn_sane_jax_triton_device, jnp.float32
    )
...
@pytest.mark.jax
def test_dn_triton_sane_single_step(delta_net_sane_inputs, dn_sane_jax_triton_device):
    """JAX-Triton SANE 单步 RNN 与 native 参考对齐（do_sane=1/0 两种情形）。"""
    for do_sane_val in (1.0, 0.0):
        q = _to_jax_tensor(
            delta_net_sane_inputs["q"][:, 0], dn_sane_jax_triton_device, jnp.bfloat16
        )
        k = _to_jax_tensor(
            delta_net_sane_inputs["k"][:, 0], dn_sane_jax_triton_device, jnp.bfloat16
        )
        v = _to_jax_tensor(
            delta_net_sane_inputs["v"][:, 0], dn_sane_jax_triton_device, jnp.bfloat16
        )
        beta = _to_jax_tensor(
            delta_net_sane_inputs["beta"][:, 0], dn_sane_jax_triton_device, jnp.float32
        )
        tau = _to_jax_tensor(
            delta_net_sane_inputs["tau"][:, 0, :],
            dn_sane_jax_triton_device,
            jnp.float32,
        )
        do_sane = jnp.full(
        ...
        out_triton, state_triton = dn_triton_single_step(
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_bfloat16_io(delta_net_sane_inputs, dn_sane_jax_triton_device):
    """bfloat16 I/O 下 JAX-Triton SANE 训练算子仍与 native 参考对齐。"""
    q, k, v, beta, tau, mask, h0 = _prepare_sane_inputs(
        delta_net_sane_inputs, dn_sane_jax_triton_device, dtype="bfloat16"
    )
...
@pytest.mark.jax
def test_dn_triton_sane_head_sharding(delta_net_sane_inputs, dn_sane_jax_triton_device):
    ...
    q = _to_jax_tensor(
        delta_net_sane_inputs["q"], dn_sane_jax_triton_device, jnp.bfloat16
    )
    k = _to_jax_tensor(
        delta_net_sane_inputs["k"], dn_sane_jax_triton_device, jnp.bfloat16
    )
    v = _to_jax_tensor(
        delta_net_sane_inputs["v"], dn_sane_jax_triton_device, jnp.bfloat16
    )
    beta = _to_jax_tensor(
        delta_net_sane_inputs["beta"], dn_sane_jax_triton_device, jnp.float32
    )
    tau = _to_jax_tensor(
        delta_net_sane_inputs["tau"], dn_sane_jax_triton_device, jnp.float32
    )
    mask = _to_jax_tensor(
        delta_net_sane_inputs["mask"], dn_sane_jax_triton_device, jnp.float32
    )
    h0 = _to_jax_tensor(
        delta_net_sane_inputs["h0"], dn_sane_jax_triton_device, jnp.float32
    )
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_forward_chunk_size(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    ...
    tau, mask = _make_tau_mask_for_chunk_size(
        delta_net_sane_inputs, chunk_size, dn_sane_jax_triton_device
    )
...
@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_backward_chunk_size(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
...
@pytest.mark.jax
def test_dn_triton_sane_inference_chunk_size(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
...
@pytest.mark.jax
def test_dn_triton_sane_no_mask_fwd_matches_native(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    ...
    assert_allclose_with_stats(
        out_ref, out_triton, "no_mask triton vs native output", atol=1e-2, rtol=1e-2
    )


@pytest.mark.jax
@pytest.mark.slow
def test_dn_triton_sane_no_mask_bwd_matches_native(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    """no-mask 内核反向（含 tau 梯度）与 native 无条件分支对拍（fp32）。"""
    q, k, v, beta, tau, _, h0 = _prepare_sane_inputs(
        delta_net_sane_inputs, dn_sane_jax_triton_device, dtype="float32"
    )
    ...
    triton_grads = jax.grad(
        lambda q, k, v, beta, tau, h0: _sane_loss_fn(
            dn_triton_recurrent, q, k, v, beta, tau, None, h0
        ),
        argnums=(0, 1, 2, 3, 4, 5),
    )(q, k, v, beta, tau, h0)
...
@pytest.mark.jax
def test_dn_triton_sane_output_final_state_false_ignores_mask(
    delta_net_sane_inputs, dn_sane_jax_triton_device
):
    ...
    out_with_mask, _ = dn_triton_recurrent(
        q,
        k,
        v,
        beta,
    ...
    out_no_mask, _ = dn_triton_recurrent(
        q,
        k,
        v,
        beta,
