...
@pytest.mark.jax
def test_gdn_pallas_sane_head_sharding(gdn_sane_inputs, gdn_sane_jax_device):
    ...
    q = _to_jax_tensor(gdn_sane_inputs["q"], gdn_sane_jax_device, jnp.bfloat16)
    k = _to_jax_tensor(gdn_sane_inputs["k"], gdn_sane_jax_device, jnp.bfloat16)
    v = _to_jax_tensor(gdn_sane_inputs["v"], gdn_sane_jax_device, jnp.bfloat16)
    g = _to_jax_tensor(gdn_sane_inputs["g"], gdn_sane_jax_device, jnp.float32)
    beta = _to_jax_tensor(gdn_sane_inputs["beta"], gdn_sane_jax_device, jnp.float32)
    tau = _to_jax_tensor(gdn_sane_inputs["tau"], gdn_sane_jax_device, jnp.float32)
    mask = _to_jax_tensor(gdn_sane_inputs["mask"], gdn_sane_jax_device, jnp.float32)
    h0 = _to_jax_tensor(gdn_sane_inputs["h0"], gdn_sane_jax_device, jnp.float32)
...
def test_gdn_pallas_sane_forward_chunk_size(gdn_sane_inputs, gdn_sane_jax_device):
    ...
    tau, mask = _make_tau_mask_for_chunk_size(
        gdn_sane_inputs, chunk_size, gdn_sane_jax_device
    )
...
@pytest.mark.jax
@pytest.mark.slow
def test_gdn_pallas_sane_backward_chunk_size(gdn_sane_inputs, gdn_sane_jax_device):
    ...
    pallas_grads = jax.grad(
        lambda q, k, v, g, beta, tau, h0: _sane_loss_fn(
            gdn_pallas_recurrent, q, k, v, g, beta, tau, mask, h0, chunk_size=chunk_size
        ),
        argnums=(0, 1, 2, 3, 4, 5, 6),
    )(q, k, v, g, beta, tau, h0)
...
@pytest.mark.jax
def test_gdn_pallas_sane_inference_chunk_size(gdn_sane_inputs, gdn_sane_jax_device):
...
@pytest.mark.jax
def test_gdn_pallas_sane_no_mask_fwd_matches_native(
    gdn_sane_inputs, gdn_sane_jax_device
):
...
@pytest.mark.jax
@pytest.mark.slow
def test_gdn_pallas_sane_no_mask_bwd_matches_native(
    gdn_sane_inputs, gdn_sane_jax_device
):
    """no-mask 内核反向（含 tau 梯度）与 native 无条件分支对拍（fp32）。"""
    q, k, v, g, beta, tau, _, h0 = _prepare_sane_inputs(
        gdn_sane_inputs, gdn_sane_jax_device, dtype="float32"
    )
    ...
    pallas_grads = jax.grad(
        lambda q, k, v, g, beta, tau, h0: _sane_loss_fn(
            gdn_pallas_recurrent, q, k, v, g, beta, tau, None, h0
        ),
        argnums=(0, 1, 2, 3, 4, 5, 6),
    )(q, k, v, g, beta, tau, h0)
...
@pytest.mark.jax
def test_gdn_pallas_sane_output_final_state_false_ignores_mask(
    gdn_sane_inputs, gdn_sane_jax_device
):
