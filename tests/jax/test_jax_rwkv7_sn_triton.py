...
def test_rwkv7_sn_triton_forward_state(
    ...
    y_c, s_c = rwkv7_sn_jax_triton_op(
        r=r_c,
        w=w_c,
        k=k_c,
        v=v_c,
...
def test_rwkv7_sn_triton_backward(
    ...
    triton_grads = jax.grad(
        lambda *p: loss(rwkv7_sn_jax_triton_op, p), argnums=range(9)
    )(w, r, k, v, a, b, tau, mask, h0)
...
def test_rwkv7_sn_triton_no_mask_backward(
    ...
    triton_grads = jax.grad(
        lambda *p: loss(rwkv7_sn_jax_triton_op, p), argnums=range(8)
    )(w, r, k, v, a, b, tau, h0)
...
def test_rwkv7_sn_triton_no_mask_y_matches_all_one(
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        y_no_mask, s_no_mask = rwkv7_sn_jax_triton_op(
            r=r,
            w=w,
            k=k,
            v=v,
    ...
    y_all_one, s_all_one = rwkv7_sn_jax_triton_op(
        r=r,
        w=w,
        k=k,
        v=v,
...
@pytest.mark.jax
def test_rwkv7_sn_triton_head_sharding(rwkv7_sn_jax_triton_op, rwkv7_sn_inputs):
