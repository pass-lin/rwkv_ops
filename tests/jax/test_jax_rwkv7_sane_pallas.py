...
def _prepare_inputs(
    ...
    Args:
        ...
        rng: np.random.Generator 或 None。为 None 时使用 fixture 自带的 tau
            与全 1 mask（对应 chunk_size=16）；否则按该 rng 重新生成 tau/mask。
        chunk_size: int, chunk 长度。rng 为 None 时固定为 16；否则用于生成
            tau/mask 的形状。
    ...
    if rng is None:
        ...
        tau = _to_jax(tau.astype(np.float32), "float32")
...
def _get_pallas_sane_op(K, chunk_size):
    """构造指定 chunk_size 的 RWKV-7-SANE Pallas 训练算子。
    ...
    op, _ = get_generalized_delta_rule_sane(
        HEAD_SIZE=K, KERNEL_TYPE="native", chunk_size=chunk_size
    )
    return op
...
def test_rwkv7_sane_pallas_backward_chunk32(
    ...
    pallas_grads = jax.grad(lambda *p: loss(pallas_op, p), argnums=range(9))(
        w, r, k, v, a, b, tau, mask, h0
    )
...
def test_rwkv7_sane_pallas_no_mask_backward_chunk32(
    ...
    pallas_grads = jax.grad(lambda *p: loss(pallas_op, p), argnums=range(8))(
        w, r, k, v, a, b, tau, h0
    )
