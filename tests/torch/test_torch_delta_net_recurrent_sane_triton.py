...
from rwkv_ops.delta_net_recurrent_sane.torch_triton_kernel import (
...
def _dn_sane_grads(fn, q, k, v, beta, tau, mask, h0):
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_recurrent_matches_native(
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_bwd_matches_native(delta_net_sane_inputs, dn_sane_cuda_device):
    ...
    grads_tri = _dn_sane_grads(dn_triton_recurrent, q, k, v, beta, tau, mask, h0)
...
@pytest.mark.torch
def test_dn_sane_triton_inference_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
def test_dn_sane_triton_single_step_do_sane_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
def test_dn_sane_triton_single_step_do_skip_matches_original_gdn(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
def test_dn_sane_triton_no_final_state(delta_net_sane_inputs, dn_sane_cuda_device):
...
@pytest.mark.torch
def test_dn_sane_triton_mask_all_ones_matches_no_mask(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    ...
    out_masked, state_masked = dn_triton_recurrent(
        q,
        k,
        v,
        beta,
    ...
    with pytest.warns(UserWarning, match="mask is None"):
        out_uncond, state_uncond = dn_triton_recurrent(
...
@pytest.mark.torch
def test_dn_sane_triton_mask_all_zeros_matches_original_gdn(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    ...
    out_sane, state_sane = dn_triton_recurrent(
        q,
        k,
        v,
        beta,
...
@pytest.mark.torch
def test_dn_sane_triton_rejects_arbitrary_length(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
def test_dn_sane_triton_bfloat16(delta_net_sane_inputs, dn_sane_cuda_device):
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_recurrent_chunk_size_32_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    """chunk_size=32 时 Triton recurrent SANE 训练算子前向与 native 参考对齐。"""
    q = _to_cuda_tensor(delta_net_sane_inputs["q"], dn_sane_cuda_device)
    k = _to_cuda_tensor(delta_net_sane_inputs["k"], dn_sane_cuda_device)
    v = _to_cuda_tensor(delta_net_sane_inputs["v"], dn_sane_cuda_device)
    beta = _to_cuda_tensor(delta_net_sane_inputs["beta"], dn_sane_cuda_device)
    ...
    op_tri = get_delta_net_recurrent_sane(KERNEL_TYPE="triton", chunk_size=32)
    ...
    assert_allclose_with_stats(
        out_ref, out_tri, "chunk_size=32 triton vs native output", atol=1e-4, rtol=1e-3
    )
    assert_allclose_with_stats(
        state_ref,
        state_tri,
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_bwd_chunk_size_32_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    """chunk_size=32 时 Triton recurrent SANE 反向梯度与 native Keras autograd 对齐。"""
    q = _to_cuda_tensor(delta_net_sane_inputs["q"], dn_sane_cuda_device)
    k = _to_cuda_tensor(delta_net_sane_inputs["k"], dn_sane_cuda_device)
    v = _to_cuda_tensor(delta_net_sane_inputs["v"], dn_sane_cuda_device)
    beta = _to_cuda_tensor(delta_net_sane_inputs["beta"], dn_sane_cuda_device)
    ...
    grads_tri = _dn_sane_grads(op_tri, q, k, v, beta, tau32, mask32, h0)
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_inference_chunk_size_32_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    """chunk_size=32 时 Triton recurrent SANE 推理算子前向与 native 参考对齐。"""
    q = _to_cuda_tensor(delta_net_sane_inputs["q"], dn_sane_cuda_device)
    k = _to_cuda_tensor(delta_net_sane_inputs["k"], dn_sane_cuda_device)
    v = _to_cuda_tensor(delta_net_sane_inputs["v"], dn_sane_cuda_device)
    beta = _to_cuda_tensor(delta_net_sane_inputs["beta"], dn_sane_cuda_device)
    ...
    op_tri = get_delta_net_recurrent_sane_inference(KERNEL_TYPE="triton", chunk_size=32)
    ...
    assert_allclose_with_stats(
        out_ref,
        out_tri,
        "chunk_size=32 triton inference vs native output",
        atol=1e-4,
        rtol=1e-3,
    )
    assert_allclose_with_stats(
        state_ref,
        state_tri,
        "chunk_size=32 triton inference vs native state",
        atol=1e-4,
        rtol=1e-3,
    )
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_no_final_state_chunk_size_32(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    ...
    assert_allclose_with_stats(
        out_ref,
        out_tri,
        "chunk_size=32 no-state triton vs native output",
        atol=1e-4,
        rtol=1e-3,
    )
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_mask_all_ones_matches_no_mask_chunk_size_32(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_mask_all_zeros_matches_original_gdn_chunk_size_32(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_bfloat16_chunk_size_32(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    """chunk_size=32 时 bfloat16 I/O 下 Triton 结果仍与 native float32 参考一致。"""
    pytest.importorskip("torch").bfloat16  # noqa: B015
    ...
    assert_allclose_with_stats(
        out_ref,
        out_tri,
        "chunk_size=32 bf16 triton vs native output",
        atol=1e-2,
        rtol=1e-2,
    )
    assert_allclose_with_stats(
        state_ref,
        state_tri,
        "chunk_size=32 bf16 triton vs native state",
        atol=1e-2,
        rtol=1e-2,
    )
...
@pytest.mark.torch
def test_dn_sane_triton_no_mask_fwd_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
...
@pytest.mark.torch
@pytest.mark.slow
def test_dn_sane_triton_no_mask_bwd_matches_native(
    delta_net_sane_inputs, dn_sane_cuda_device
):
    ...
    grads_tri = _run_and_grad(dn_triton_recurrent)
...
@pytest.mark.torch
def test_dn_sane_triton_output_final_state_false_ignores_mask(
    delta_net_sane_inputs, dn_sane_cuda_device
):
