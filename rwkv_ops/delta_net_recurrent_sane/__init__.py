    ...
    if keras.config.backend() == "torch":
        if KERNEL_TYPE == "cuda":
            ...
            if torch.cuda.is_available():
                from .torch_cuda_kernel.delta_net_recurrent_sane_torch import (
                    delta_net_recurrent_sane as cuda_op,
                )
            ...
            from .torch_triton_kernel import (
                delta_net_recurrent_sane as triton_op,
            )
        ...
        if KERNEL_TYPE == "cuda":
            ...
            if jax.devices()[0].platform == "gpu":
                from .jax_cuda_kernel.delta_net_recurrent_sane_jax import (
                    delta_net_recurrent_sane as cuda_op,
                )
        ...
        if _use_jax_pallas(KERNEL_TYPE):
            from .jax_pallas_kernel import delta_net_recurrent_sane as pallas_op
...
def get_delta_net_recurrent_sane_inference(KERNEL_TYPE="native", chunk_size: int = 16):
    ...
    if keras.config.backend() == "torch":
        if KERNEL_TYPE == "cuda":
            ...
            if torch.cuda.is_available():
                from .torch_cuda_kernel.delta_net_recurrent_sane_torch import (
                    delta_net_recurrent_sane_inference as cuda_op,
                )

                return functools.partial(cuda_op, chunk_size=chunk_size)
        elif KERNEL_TYPE in ("native", "triton") and _use_triton(KERNEL_TYPE):
            from .torch_triton_kernel import (
                delta_net_recurrent_sane_inference as triton_op,
            )
        ...
        if _use_jax_pallas(KERNEL_TYPE):
            from .jax_pallas_kernel import (
                delta_net_recurrent_sane_inference as pallas_op,
            )
...
def get_delta_net_recurrent_sane_single_step(
    ...
    if keras.config.backend() == "torch":
        if KERNEL_TYPE == "cuda":
            ...
            if torch.cuda.is_available():
                from .torch_cuda_kernel.delta_net_recurrent_sane_torch import (
                    delta_net_recurrent_sane_single_step as cuda_op,
                )

                return functools.partial(cuda_op, chunk_size=chunk_size)
        elif KERNEL_TYPE in ("native", "triton") and _use_triton(KERNEL_TYPE):
            from .torch_triton_kernel import (
                delta_net_recurrent_sane_single_step as triton_op,
            )
        ...
        if _use_jax_pallas(KERNEL_TYPE):
            from .jax_pallas_kernel import (
                delta_net_recurrent_sane_single_step as pallas_op,
            )
