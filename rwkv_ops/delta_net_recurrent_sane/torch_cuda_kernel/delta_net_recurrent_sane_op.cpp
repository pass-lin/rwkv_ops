// PyTorch C++ 绑定：将 DeltaNet recurrent SANE CUDA kernel 包装为
// torch.ops。
...
extern template void cuda_dn_sane_forward<bf>(int, int, int, float, const bf *,
                                              ...
                                              float *, float *);
extern template void
cuda_dn_sane_forward<float>(int, int, int, float, const float *, const float *,
                            const float *, const float *, const float *,
                            const float *, const float *, float *, float *,
                            float *, float *, float *, float *);
extern template void cuda_dn_sane_backward<bf>(
    ...
    float *, float *, float *);
extern template void cuda_dn_sane_backward<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, float *,
    float *, float *, float *, float *, float *);
extern template void
cuda_dn_sane_forward_inference<bf>(int, int, int, float, const bf *, const bf *,
                                   const bf *, const float *, const float *,
                                   const float *, const float *, bf *, float *);
extern template void cuda_dn_sane_forward_inference<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, float *,
    float *);
extern template void cuda_dn_sane_single_step<bf>(int, int, float, const bf *,
                                                  const bf *, const bf *,
                                                  const float *, const float *,
                                                  const float *, const float *,
                                                  bf *, float *);
...
void forward(torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
  ...
  if (q.scalar_type() == at::kBFloat16) {
    cuda_dn_sane_forward<bf>(
        ...
        (const float *)mask.data_ptr(), (const float *)h0.data_ptr(),
...
void backward(torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
              torch::Tensor &beta, torch::Tensor &tau, torch::Tensor &mask,
              torch::Tensor &dout, torch::Tensor &dht, torch::Tensor &kv_mem,
              torch::Tensor &inv_q, torch::Tensor &inv_k, torch::Tensor &chkp,
              double scale, torch::Tensor &dq, torch::Tensor &dk,
              torch::Tensor &dv, torch::Tensor &dbeta, torch::Tensor &dtau,
              torch::Tensor &dh0) {
  int B = q.sizes()[0], H = q.sizes()[1], T = q.sizes()[2];
  if (q.scalar_type() == at::kBFloat16) {
    cuda_dn_sane_backward<bf>(
        B, T, H, (float)scale, (const bf *)q.data_ptr(),
        (const bf *)k.data_ptr(), (const bf *)v.data_ptr(),
        (const float *)beta.data_ptr(), (const float *)tau.data_ptr(),
        (const float *)mask.data_ptr(), (const bf *)dout.data_ptr(),
        ...
        (float *)dk.data_ptr(), (float *)dv.data_ptr(),
        (float *)dbeta.data_ptr(), (float *)dtau.data_ptr(),
        (float *)dh0.data_ptr());
  } else if (q.scalar_type() == at::kFloat) {
    cuda_dn_sane_backward<float>(
        B, T, H, (float)scale, (const float *)q.data_ptr(),
        (const float *)k.data_ptr(), (const float *)v.data_ptr(),
        (const float *)beta.data_ptr(), (const float *)tau.data_ptr(),
        (const float *)mask.data_ptr(), (const float *)dout.data_ptr(),
        (const float *)dht.data_ptr(), (const float *)kv_mem.data_ptr(),
        (const float *)inv_q.data_ptr(), (const float *)inv_k.data_ptr(),
        (const float *)chkp.data_ptr(), (float *)dq.data_ptr(),
        (float *)dk.data_ptr(), (float *)dv.data_ptr(),
...
void forward_inference(torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
  ...
  int B = q.sizes()[0], H = q.sizes()[1], T = q.sizes()[2];
  if (q.scalar_type() == at::kBFloat16) {
    cuda_dn_sane_forward_inference<bf>(
        B, T, H, (float)scale, (const bf *)q.data_ptr(),
        (const bf *)k.data_ptr(), (const bf *)v.data_ptr(),
        (const float *)beta.data_ptr(), (const float *)tau.data_ptr(),
        (const float *)mask.data_ptr(), (const float *)h0.data_ptr(),
    ...
    cuda_dn_sane_forward_inference<float>(
        B, T, H, (float)scale, (const float *)q.data_ptr(),
        (const float *)k.data_ptr(), (const float *)v.data_ptr(),
        (const float *)beta.data_ptr(), (const float *)tau.data_ptr(),
        (const float *)mask.data_ptr(), (const float *)h0.data_ptr(),
...
#define TORCH_LIBRARY_NAME delta_net_recurrent_sane_cuda
#endif
TORCH_LIBRARY(TORCH_LIBRARY_NAME, m) {
  m.def("forward(Tensor q, Tensor k, Tensor v, Tensor beta, "
