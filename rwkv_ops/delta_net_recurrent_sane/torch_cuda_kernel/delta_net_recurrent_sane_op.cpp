...
extern template void cuda_dn_sane_forward<float>(
...
extern template void cuda_dn_sane_forward_inference<bf>(
    int, int, int, float, const bf *, const bf *, const bf *, 
...
extern template void cuda_dn_sane_forward_inference<float>(
    ...
    const float *, const float *, const float *, const float *, const float *,
    float *, float *);
extern template void cuda_dn_sane_single_step<bf>(
    int, int, float, const bf *, const bf *, const bf *, 
    const float *, const float *, const float *, const float *, bf *, float *);
extern template void
...
void forward(torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
  ...
  if (q.scalar_type() == at::kBFloat16) {
    ...
    TORCH_CHECK(false, "delta_net_recurrent_sane cuda kernel only supports bfloat16 "
                       "or float32 inputs");
  }
}
...
void single_step(torch::Tensor &q, torch::Tensor &k, torch::Tensor &v,
  ...
  if (q.scalar_type() == at::kBFloat16) {
    cuda_dn_sane_single_step<bf>(
        ...
        (const bf *)v.data_ptr(),         (const float *)beta.data_ptr(), (const float *)tau.data_ptr(),
