"""JAX 版 DeltaNet recurrent SANE Pallas kernel 封装。"""
...
BWD_RULE_NO_MASK = (
    "b n t k, b n t k, b n t v, b n t, b n t, b n t v, b n k v, "
    "b n t v, b n t, b n c k v, b n k v, b n c -> "
    "b n t k, b n t k, b n t v, b n t, b n t, b n k v, b n c"
)
INF_RULE_NO_MASK = (
...
def _make_delta_net_recurrent_sane_fwd_kernel(chunk_size: int):
    """构造 DeltaNet recurrent SANE 训练前向 Pallas kernel。
    ...
    def _delta_net_recurrent_sane_fwd_kernel(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
    ...
    return _delta_net_recurrent_sane_fwd_kernel


def _make_delta_net_recurrent_sane_fwd_kernel_no_mask(chunk_size: int):
    """构造 DeltaNet recurrent SANE 训练前向 Pallas kernel（无 mask）。

    返回的 kernel 在 grid (B, N) 上运行，每个 program 处理一个 (batch, head)。
    chunk 内静态展开 chunk_size 步，保存 SANE 之前的 state checkpoint；
    chunk 边界无条件执行 SANE，不读取 mask。
    """

    def _delta_net_recurrent_sane_fwd_kernel_no_mask(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
        ...
        def chunk_body(c, state):
            ...
            tau_safe = jnp.maximum(tau_v, 1e-6)
            u = state / tau_safe
            return tau_safe * jnp.tanh(u)

        jax.lax.fori_loop(0, num_chunks, chunk_body, state)

    return _delta_net_recurrent_sane_fwd_kernel_no_mask


def _make_delta_net_recurrent_sane_bwd_kernel(chunk_size: int):
    """构造 DeltaNet recurrent SANE 训练反向 Pallas kernel。

    返回的 kernel 在 grid (B, N) 上运行，每个 program 处理一个 (batch, head)。
    在每个 chunk 边界先应用 SANE 反向，再回传 chunk 内 chunk_size 步的 DeltaNet 梯度。
    """

    def _delta_net_recurrent_sane_bwd_kernel(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
    ...
    return _delta_net_recurrent_sane_bwd_kernel


def _make_delta_net_recurrent_sane_bwd_kernel_no_mask(chunk_size: int):
    """构造 DeltaNet recurrent SANE 训练反向 Pallas kernel（无 mask）。

    返回的 kernel 在 grid (B, N) 上运行，每个 program 处理一个 (batch, head)。
    在每个 chunk 边界先应用 SANE 反向（无条件累加 dtau），再回传 chunk 内梯度。
    """

    def _delta_net_recurrent_sane_bwd_kernel_no_mask(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
        ...
        def chunk_body(c_rev, dS):
            ...
            # 无 mask 时无条件回传 sech2 梯度并累加 dtau。
            tau_v = tau_ref[b, h, c].astype(jnp.float32)
            tau_safe = jnp.maximum(tau_v, 1e-6)
            u = state / tau_safe
            tnh = jnp.tanh(u)
            sech2 = 1.0 - tnh * tnh
            dtau_ref[b, h, c] = jnp.sum(dS * (tnh - u * sech2))
            dS = dS * sech2
    ...
    return _delta_net_recurrent_sane_bwd_kernel_no_mask


def _make_delta_net_recurrent_sane_inf_kernel(chunk_size: int):
    """构造 DeltaNet recurrent SANE 推理前向 Pallas kernel。

    与训练前向数学一致，但不做反向保存；支持任意序列长度 T。
    """

    def _delta_net_recurrent_sane_inf_kernel(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
    ...
    return _delta_net_recurrent_sane_inf_kernel


def _make_delta_net_recurrent_sane_inf_kernel_no_mask(chunk_size: int):
    """构造 DeltaNet recurrent SANE 推理前向 Pallas kernel（无 mask）。

    与训练前向数学一致，但不做反向保存；chunk 边界无条件 SANE，支持任意 T。
    """

    def _delta_net_recurrent_sane_inf_kernel_no_mask(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
    ...
    return _delta_net_recurrent_sane_inf_kernel_no_mask


def _delta_net_recurrent_sane_single_step_kernel(
    q_ref,
    k_ref,
    v_ref,
    beta_ref,
    ...
    """DeltaNet recurrent SANE 单步 RNN Pallas kernel。

    输入无时间维：q/k 为 [B, N, K]，v/o 为 [B, N, V]，beta 为 [B, N]。
    """
    b = pl.program_id(0)
    h = pl.program_id(1)
...
def _delta_net_recurrent_sane_fwd_pallas_call(
    q, k, v, beta, tau, mask, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_sane_fwd_{chunk_size}",
        _make_delta_net_recurrent_sane_fwd_kernel(chunk_size),
...
def _delta_net_recurrent_sane_fwd_warmup(q, k, v, beta, tau, mask, h0, chunk_size: int):
    B, N, T, K = q.shape
    ensure_config(
        f"delta_net_recurrent_sane_fwd_{chunk_size}",
        _make_delta_net_recurrent_sane_fwd_kernel(chunk_size),
...
_delta_net_recurrent_sane_fwd_spmd.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_fwd_pallas_call),
)


def _delta_net_recurrent_sane_fwd_pallas_call_no_mask(
    q, k, v, beta, tau, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_sane_fwd_no_mask_{chunk_size}",
        _make_delta_net_recurrent_sane_fwd_kernel_no_mask(chunk_size),
        _fwd_out_shape(q, v, chunk_size),
        (B, N),
        (q, k, v, beta, tau, h0),
    )
...
def _delta_net_recurrent_sane_fwd_warmup_no_mask(
    q, k, v, beta, tau, h0, chunk_size: int
):
    B, N, T, K = q.shape
    ensure_config(
...
def _delta_net_recurrent_sane_fwd_spmd_no_mask(q, k, v, beta, tau, h0, chunk_size: int):
    return _delta_net_recurrent_sane_fwd_pallas_call_no_mask(
...
_delta_net_recurrent_sane_fwd_spmd_no_mask.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_fwd_pallas_call_no_mask),
)
...
def _delta_net_recurrent_sane_bwd_pallas_call(
    q,
    k,
    v,
    beta,
    ...
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_sane_bwd_{chunk_size}",
        _make_delta_net_recurrent_sane_bwd_kernel(chunk_size),
...
def _delta_net_recurrent_sane_bwd_warmup(
    q,
    k,
    v,
    beta,
...
def _delta_net_recurrent_sane_bwd_spmd(
    ...
    return _delta_net_recurrent_sane_bwd_pallas_call(
        q,
        k,
        v,
        beta,
...
_delta_net_recurrent_sane_bwd_spmd.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_bwd_pallas_call),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(7,))
def _delta_net_recurrent_sane_train(q, k, v, beta, tau, mask, h0, chunk_size: int):
    _delta_net_recurrent_sane_fwd_warmup(q, k, v, beta, tau, mask, h0, chunk_size)
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = (
        _delta_net_recurrent_sane_fwd_spmd(q, k, v, beta, tau, mask, h0, chunk_size)
    )
...
def _dn_train_bwd(chunk_size: int, res, grads):
    q, k, v, beta, tau, mask, kv_mem, inv_norm_q, inv_norm_k, state_chkp, h0 = res
    dy, dht = grads
    ...
    _delta_net_recurrent_sane_bwd_warmup(
        q,
        k,
        v,
        beta,
...
def _delta_net_recurrent_sane_bwd_pallas_call_no_mask(
    q,
    k,
    v,
    beta,
    ...
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_sane_bwd_no_mask_{chunk_size}",
        _make_delta_net_recurrent_sane_bwd_kernel_no_mask(chunk_size),
        _bwd_out_shape(q, v, chunk_size),
        (B, N),
        (q, k, v, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, chkp, h0, tau),
    )


def _delta_net_recurrent_sane_bwd_warmup_no_mask(
    q,
    k,
    v,
    beta,
...
def _delta_net_recurrent_sane_bwd_spmd_no_mask(
    ...
    return _delta_net_recurrent_sane_bwd_pallas_call_no_mask(
        q,
        k,
        v,
        beta,
...
_delta_net_recurrent_sane_bwd_spmd_no_mask.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_bwd_pallas_call_no_mask),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(6,))
def _delta_net_recurrent_sane_train_no_mask(q, k, v, beta, tau, h0, chunk_size: int):
    _delta_net_recurrent_sane_fwd_warmup_no_mask(q, k, v, beta, tau, h0, chunk_size)
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = (
...
def _dn_train_bwd_no_mask(chunk_size: int, res, grads):
    q, k, v, beta, tau, kv_mem, inv_norm_q, inv_norm_k, state_chkp, h0 = res
    dy, dht = grads
    dy = jnp.asarray(dy, q.dtype)
    if dht is None:
        B, N, T, K = q.shape
    ...
    _delta_net_recurrent_sane_bwd_warmup_no_mask(
        q,
        k,
        v,
        beta,
...
def _delta_net_recurrent_sane_inf_pallas_call(
    q, k, v, beta, tau, mask, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_sane_inf_{chunk_size}",
        _make_delta_net_recurrent_sane_inf_kernel(chunk_size),
        _inf_out_shape(q, v),
        (B, N),
        (q, k, v, beta, tau, mask, h0),
    )
...
def _delta_net_recurrent_sane_inf_warmup(q, k, v, beta, tau, mask, h0, chunk_size: int):
    B, N, T, K = q.shape
    ensure_config(
        f"delta_net_recurrent_sane_inf_{chunk_size}",
        _make_delta_net_recurrent_sane_inf_kernel(chunk_size),
...
_delta_net_recurrent_sane_inf_spmd.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_inf_pallas_call),
)


def _delta_net_recurrent_sane_inf_pallas_call_no_mask(
    q, k, v, beta, tau, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_sane_inf_no_mask_{chunk_size}",
        _make_delta_net_recurrent_sane_inf_kernel_no_mask(chunk_size),
        _inf_out_shape(q, v),
        (B, N),
        (q, k, v, beta, tau, h0),
    )
...
def _delta_net_recurrent_sane_inf_warmup_no_mask(
    q, k, v, beta, tau, h0, chunk_size: int
):
    B, N, T, K = q.shape
    ensure_config(
...
def _delta_net_recurrent_sane_inf_spmd_no_mask(q, k, v, beta, tau, h0, chunk_size: int):
    return _delta_net_recurrent_sane_inf_pallas_call_no_mask(
        q, k, v, beta, tau, h0, chunk_size
    )
...
_delta_net_recurrent_sane_inf_spmd_no_mask.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_inf_pallas_call_no_mask),
)
...
def _delta_net_recurrent_sane_single_step_pallas_call(q, k, v, beta, tau, do_sane, h0):
    B, N, K = q.shape
    return launch(
        "delta_net_recurrent_sane_single_step",
        _delta_net_recurrent_sane_single_step_kernel,
...
def _delta_net_recurrent_sane_single_step_warmup(q, k, v, beta, tau, do_sane, h0):
    B, N, K = q.shape
    ensure_config(
        "delta_net_recurrent_sane_single_step",
        _delta_net_recurrent_sane_single_step_kernel,
...
@custom_partitioning
def _delta_net_recurrent_sane_single_step_spmd(q, k, v, beta, tau, do_sane, h0):
    return _delta_net_recurrent_sane_single_step_pallas_call(
        q, k, v, beta, tau, do_sane, h0
    )
...
_delta_net_recurrent_sane_single_step_spmd.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_sane_single_step_pallas_call),
)
...
def delta_net_recurrent_sane(
    ...
    final_state = _apply_sane_to_final_state(
        final_state, tau, mask=mask_arr if use_mask else None
    )
    return out, final_state
...
def delta_net_recurrent_sane_inference(
    ...
    if use_mask:
        ...
        _delta_net_recurrent_sane_inf_warmup(
            q, k, v, beta, tau, mask_arr, h0, chunk_size
        )
        ...
        # 无 mask 路径：chunk 边界无条件执行 SANE，不构造占位 mask。
        _delta_net_recurrent_sane_inf_warmup_no_mask(q, k, v, beta, tau, h0, chunk_size)
...
def delta_net_recurrent_sane_single_step(
    ...
    _delta_net_recurrent_sane_single_step_warmup(q, k, v, beta, tau, do_sane, h0)
