"""DeltaNet recurrent SANE Keras 参考实现。"""
...
def _delta_net_recurrent_sane_step(
    t,
    inputs,
    q,
    k,
    ...
    """DeltaNet recurrent SANE 单步递推（供 fori_loop 使用）。

    Args:
        t: 当前时间步索引。
        inputs: [state, out] 元组。
        q, k, v, beta: [B, H, T, *] 的 float32 张量。
...
def delta_net_recurrent_sane(
    ...
    """带 State Anomaly Neutralization 的 DeltaNet recurrent 原生实现。
    ...
    state, out = ops.fori_loop(
        0,
        T,
        lambda t, inputs: _delta_net_recurrent_sane_step(
            t,
            inputs,
            q,
            k,
...
def delta_net_recurrent_sane_inference(
    ...
    """DeltaNet recurrent SANE 推理原生封装（无梯度）。

    当前阶段与 `delta_net_recurrent_sane` 数学等价，仅用于保持 API 一致性。
    ...
    """
    return delta_net_recurrent_sane(
        q,
        k,
        v,
        beta,
...
def _delta_net_recurrent_sane_single_step_core(
    q, k, v, beta, tau, do_sane, state, scale
):
    """DeltaNet recurrent SANE 单步核心。
...
def delta_net_recurrent_sane_single_step(
    ...
    """DeltaNet recurrent SANE 单步 RNN 原生实现。
    ...
    out, state = _delta_net_recurrent_sane_single_step_core(
        q, k, v, beta, tau, do_sane, state, scale
    )
