"""JAX 版 DeltaNet recurrent SANE Triton kernel 封装。"""
...
FWD_RULE = (
    "b n t k, b n t k, b n t v, b n t, b n c, b c, b n k v -> "
...
BWD_RULE = (
    ...
    "b n t k, b n t k, b n t v, b n t, b n c, b n k v"
)
INF_RULE = "b n t k, b n t k, b n t v, b n t, b n c, b c, b n k v -> b n t v, b n k v"
SINGLE_RULE = "b n k, b n k, b n v, b n, b n, b, b n k v -> b n v, b n k v"
...
INF_RULE_NO_MASK = (
    "b n t k, b n t k, b n t v, b n t, b n c, b n k v -> b n t v, b n k v"
)
...
def _sharding_for_gb(qs):
    """为 beta / inv_norm [B, N, T] 构造 sharding。"""
    spec = _q_spec(qs)
    if spec is None:
        return qs
...
def _delta_net_recurrent_sane_fwd_triton_call(
    q, k, v, beta, tau, mask, h0, use_mask, chunk_size
):
    B, N, T, K = q.shape
    V = v.shape[-1]
    dtype = v.dtype
    ...
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k, _ = jt.triton_call(
        ...
        N,
        T,
        kernel=delta_net_recurrent_sane_fwd_kernel,
...
def _delta_net_recurrent_sane_fwd_spmd_impl(
    q, k, v, beta, tau, mask, h0, use_mask, chunk_size
):
    return _delta_net_recurrent_sane_fwd_triton_call(
        q, k, v, beta, tau, mask, h0, use_mask, chunk_size
    )
...
_delta_net_recurrent_sane_fwd_spmd.def_partition(
    ...
    partition=_create_partition(_delta_net_recurrent_sane_fwd_spmd_impl),
)


def _delta_net_recurrent_sane_fwd_triton_call_no_mask(
    ...
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k, _ = jt.triton_call(
        ...
        N,
        T,
        kernel=delta_net_recurrent_sane_fwd_kernel_no_mask,
        out_shape=out_shapes,
        grid=grid,
        K=K,
        V=V,
...
def _delta_net_recurrent_sane_fwd_spmd_impl_no_mask(q, k, v, beta, tau, h0, chunk_size):
    return _delta_net_recurrent_sane_fwd_triton_call_no_mask(
        q, k, v, beta, tau, h0, chunk_size
    )


_delta_net_recurrent_sane_fwd_spmd_no_mask = custom_partitioning(
    _delta_net_recurrent_sane_fwd_spmd_impl_no_mask, static_argnums=(6,)
)
_delta_net_recurrent_sane_fwd_spmd_no_mask.def_partition(
    infer_sharding_from_operands=_fwd_infer_sharding,
    sharding_rule=FWD_RULE_NO_MASK,
    partition=_create_partition(_delta_net_recurrent_sane_fwd_spmd_impl_no_mask),
)
...
def _delta_net_recurrent_sane_bwd_triton_call(
    q,
    k,
    v,
    beta,
    ...
    return jt.triton_call(
        ...
        N,
        T,
        kernel=delta_net_recurrent_sane_bwd_kernel,
        out_shape=out_shapes,
        grid=grid,
...
def _delta_net_recurrent_sane_bwd_spmd_impl(
    q,
    k,
    v,
    beta,
    ...
    return _delta_net_recurrent_sane_bwd_triton_call(
        q,
        k,
        v,
        beta,
...
_delta_net_recurrent_sane_bwd_spmd.def_partition(
    ...
    partition=_create_partition(_delta_net_recurrent_sane_bwd_spmd_impl),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(7, 8))
def _delta_net_recurrent_sane_train(q, k, v, beta, tau, mask, h0, use_mask, chunk_size):
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = (
        _delta_net_recurrent_sane_fwd_spmd(
            q, k, v, beta, tau, mask, h0, use_mask, chunk_size
        )
    )
...
def _dn_train_fwd(q, k, v, beta, tau, mask, h0, use_mask, chunk_size):
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = (
        _delta_net_recurrent_sane_fwd_spmd(
            q, k, v, beta, tau, mask, h0, use_mask, chunk_size
        )
...
def _dn_train_bwd(use_mask, chunk_size, res, grads):
    (
        q,
        k,
        v,
    ...
    dq, dk, dv, dbeta, dtau, dh0 = _delta_net_recurrent_sane_bwd_spmd(
        q,
        k,
        v,
        beta,
...
def _delta_net_recurrent_sane_bwd_triton_call_no_mask(
    q,
    k,
    v,
    beta,
    ...
    return jt.triton_call(
        ...
        N,
        T,
        kernel=delta_net_recurrent_sane_bwd_kernel_no_mask,
        out_shape=out_shapes,
        grid=grid,
        zeroed_outputs=(0, 1, 2, 3, 4, 5),
        K=K,
...
def _delta_net_recurrent_sane_bwd_spmd_impl_no_mask(
    q,
    k,
    v,
    beta,
    ...
    return _delta_net_recurrent_sane_bwd_triton_call_no_mask(
        q,
        k,
        v,
        beta,
...
_delta_net_recurrent_sane_bwd_spmd_no_mask = custom_partitioning(
    _delta_net_recurrent_sane_bwd_spmd_impl_no_mask, static_argnums=(12,)
)
_delta_net_recurrent_sane_bwd_spmd_no_mask.def_partition(
    infer_sharding_from_operands=_bwd_infer_sharding,
    sharding_rule=BWD_RULE_NO_MASK,
    partition=_create_partition(_delta_net_recurrent_sane_bwd_spmd_impl_no_mask),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(6,))
def _delta_net_recurrent_sane_train_no_mask(q, k, v, beta, tau, h0, chunk_size):
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = (
...
def _dn_train_fwd_no_mask(q, k, v, beta, tau, h0, chunk_size):
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = (
        _delta_net_recurrent_sane_fwd_spmd_no_mask(q, k, v, beta, tau, h0, chunk_size)
    )
    final_state = _apply_sane_to_final_state(state_chkp[:, :, -1, :, :], tau, mask=None)
...
def _dn_train_bwd_no_mask(chunk_size, res, grads):
    (
        q,
        k,
        v,
    ...
    dq, dk, dv, dbeta, dtau, dh0 = _delta_net_recurrent_sane_bwd_spmd_no_mask(
        q,
        k,
        v,
        beta,
...
def _delta_net_recurrent_sane_inf_triton_call(
    q, k, v, beta, tau, mask, h0, use_mask, chunk_size
):
    B, N, T, K = q.shape
    V = v.shape[-1]
    ...
    return jt.triton_call(
        ...
        N,
        T,
        kernel=delta_net_recurrent_sane_inference_fwd_kernel,
        out_shape=out_shapes,
        grid=grid,
        K=K,
        V=V,
...
def _delta_net_recurrent_sane_inf_spmd_impl(
    q, k, v, beta, tau, mask, h0, use_mask, chunk_size
):
    return _delta_net_recurrent_sane_inf_triton_call(
        q, k, v, beta, tau, mask, h0, use_mask, chunk_size
    )
...
_delta_net_recurrent_sane_inf_spmd.def_partition(
    ...
    partition=_create_partition(_delta_net_recurrent_sane_inf_spmd_impl),
)


def _delta_net_recurrent_sane_inf_triton_call_no_mask(
    q, k, v, beta, tau, h0, chunk_size
):
...
def _delta_net_recurrent_sane_inf_spmd_impl_no_mask(q, k, v, beta, tau, h0, chunk_size):
    return _delta_net_recurrent_sane_inf_triton_call_no_mask(
        q, k, v, beta, tau, h0, chunk_size
    )


_delta_net_recurrent_sane_inf_spmd_no_mask = custom_partitioning(
    _delta_net_recurrent_sane_inf_spmd_impl_no_mask, static_argnums=(6,)
)
_delta_net_recurrent_sane_inf_spmd_no_mask.def_partition(
    infer_sharding_from_operands=_inf_infer_sharding,
    sharding_rule=INF_RULE_NO_MASK,
    partition=_create_partition(_delta_net_recurrent_sane_inf_spmd_impl_no_mask),
)
...
def _delta_net_recurrent_sane_single_step_triton_call(q, k, v, beta, tau, do_sane, h0):
    B, N, K = q.shape
    V = v.shape[-1]
    dtype = v.dtype
    BV = _compute_bv(V)
    ...
    return jt.triton_call(
        ...
        B,
        N,
        kernel=delta_net_recurrent_sane_single_step_fwd_kernel,
        out_shape=out_shapes,
        grid=grid,
        K=K,
        V=V,
...
def _delta_net_recurrent_sane_single_step_spmd_impl(q, k, v, beta, tau, do_sane, h0):
    return _delta_net_recurrent_sane_single_step_triton_call(
        q, k, v, beta, tau, do_sane, h0
    )


_delta_net_recurrent_sane_single_step_spmd = custom_partitioning(
    _delta_net_recurrent_sane_single_step_spmd_impl
)
_delta_net_recurrent_sane_single_step_spmd.def_partition(
    ...
    partition=_create_partition(_delta_net_recurrent_sane_single_step_spmd_impl),
)
...
def delta_net_recurrent_sane(
    ...
    if use_mask:
        ...
        out, final_state = _delta_net_recurrent_sane_train(
            q, k, v, beta, tau, mask_arr, h0, use_mask, chunk_size
        )
    else:
