"""PyTorch 版 DeltaNet recurrent SANE Triton kernel 封装。"""
...
def _make_delta_net_recurrent_sane_triton_function(chunk_size):
    """按 chunk_size 构造 DeltaNet recurrent SANE 训练 Triton 封装类。"""

    class DeltaNetRecurrentSaneTritonFunction(torch.autograd.Function):
        """DeltaNet recurrent SANE 训练前向 Triton 封装。"""
        ...
        def forward(
            ...
            # 与 native 保持一致：仅在 output_final_state=True 时按 mask 门控 SANE，
            # 否则走无条件 SANE（mask 不参与）。
            use_mask = use_mask and output_final_state
    ...
    return DeltaNetRecurrentSaneTritonFunction


def _make_delta_net_recurrent_sane_inference_triton_function(chunk_size):
    """按 chunk_size 构造 DeltaNet recurrent SANE 推理 Triton 封装类。"""

    class DeltaNetRecurrentSaneInferenceTritonFunction(torch.autograd.Function):
        """DeltaNet recurrent SANE 推理前向 Triton 封装（无反向）。"""
    ...
    return DeltaNetRecurrentSaneInferenceTritonFunction


class DeltaNetRecurrentSaneSingleStepTritonFunction(torch.autograd.Function):
    """DeltaNet recurrent SANE 单步 RNN 前向 Triton 封装（无反向）。"""
    ...
    def forward(
        ...
        if not head_first:
            raise NotImplementedError(
                "delta_net_recurrent_sane_single_step currently only supports "
                "head_first=True."
            )
        ...
        delta_net_recurrent_sane_single_step_fwd_kernel[grid](
            q=q,
            k=k,
            v=v,
            beta=beta,
...
def delta_net_recurrent_sane(
    ...
    """DeltaNet recurrent SANE 训练算子（PyTorch Triton 实现）。
    ...
    Op = _make_delta_net_recurrent_sane_triton_function(chunk_size)
    return Op.apply(
        q,
        k,
        v,
...
def delta_net_recurrent_sane_inference(
    ...
    """DeltaNet recurrent SANE 推理算子（PyTorch Triton 实现，无梯度）。
    ...
    Op = _make_delta_net_recurrent_sane_inference_triton_function(chunk_size)
    return Op.apply(
        q,
        k,
        v,
...
def delta_net_recurrent_sane_single_step(
    ...
    """DeltaNet recurrent SANE 单步 RNN 算子（PyTorch Triton 实现）。
    ...
    return DeltaNetRecurrentSaneSingleStepTritonFunction.apply(
        q,
        k,
        v,
        beta,
