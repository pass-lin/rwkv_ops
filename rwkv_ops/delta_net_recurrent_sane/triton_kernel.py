"""DeltaNet recurrent SANE Triton kernel（共享实现）。"""
...
@triton.jit
def delta_net_recurrent_sane_fwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 训练前向 Triton kernel。
...
def _bwd_autotune_pre_hook(nargs, reset_only=False):
    ...
    for name in ("dq", "dk", "dv", "dbeta", "dh0", "dtau"):
        buf = nargs.get(name)
        if buf is None:
            continue
...
@triton.jit
def delta_net_recurrent_sane_bwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 训练反向 Triton kernel。
...
@triton.jit
def delta_net_recurrent_sane_inference_fwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 推理前向 Triton kernel。
...
@triton.jit
def delta_net_recurrent_sane_single_step_fwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 单步 RNN 前向 Triton kernel。

    每个 program 处理一个 (batch, head) 与 V 方向的一个 block。
    输入没有时间维：q/k 为 [B, H, K]，v/o 为 [B, H, V]，beta 为 [B, H]。
...
@triton.jit
def delta_net_recurrent_sane_fwd_kernel_no_mask(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 训练前向 Triton kernel（无 mask）。
...
@triton.jit
def delta_net_recurrent_sane_bwd_kernel_no_mask(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 训练反向 Triton kernel（无 mask）。
...
@triton.jit
def delta_net_recurrent_sane_inference_fwd_kernel_no_mask(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent SANE 推理前向 Triton kernel（无 mask）。
