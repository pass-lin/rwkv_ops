"""JAX 版 DeltaNet chunkwise SANE Triton kernel 封装。"""
...
def _fwd_h_sane_no_mask_call(k, w, u, tau, h0, chunk_size):
...
_fwd_h_sane_no_mask_spmd = custom_partitioning(
    _fwd_h_sane_no_mask_call, static_argnums=(5,)
)
_fwd_h_sane_no_mask_spmd.def_partition(
    infer_sharding_from_operands=lambda arg_shapes, arg_shardings: (
        _for_h(arg_shardings[0]),
        _like_q(arg_shardings[2]),
        _for_final(arg_shardings[4]),
    ),
    sharding_rule=(
        "b h t k, b h t k, b h t v, b h c, b h k v -> b h n k v, b h t v, b h k v"
    ),
    partition=_create_partition(_fwd_h_sane_no_mask_call),
)


def _fwd_h_sane_with_mask_call(k, w, u, tau, mask, h0, chunk_size):
...
_fwd_h_sane_with_mask_spmd = custom_partitioning(
    _fwd_h_sane_with_mask_call, static_argnums=(6,)
)
_fwd_h_sane_with_mask_spmd.def_partition(
    ...
    sharding_rule=(
        "b h t k, b h t k, b h t v, b h c, b c, b h k v -> b h n k v, b h t v, b h k v"
    ),
...
def _bwd_dhu_sane_no_mask_call(q, k, w, h, v_new, tau, do, dv_local, dht, chunk_size):
...
_bwd_dhu_sane_no_mask_spmd = custom_partitioning(
    _bwd_dhu_sane_no_mask_call, static_argnums=(9,)
)
_bwd_dhu_sane_no_mask_spmd.def_partition(
    infer_sharding_from_operands=lambda arg_shapes, arg_shardings: (
        _for_h(arg_shardings[0]),
        _for_final(arg_shardings[0]),
        _like_q(arg_shardings[7]),
        _for_tau(arg_shardings[0]),
    ),
    sharding_rule=(
        "b h t k, b h t k, b h t k, b h n k v, b h t v, b h c, "
...
def _bwd_dhu_sane_with_mask_call(
    q, k, w, h, v_new, tau, mask, do, dv_local, dht, chunk_size
):
...
_bwd_dhu_sane_with_mask_spmd = custom_partitioning(
    _bwd_dhu_sane_with_mask_call, static_argnums=(10,)
)
_bwd_dhu_sane_with_mask_spmd.def_partition(
    ...
    sharding_rule=(
        "b h t k, b h t k, b h t k, b h n k v, b h t v, b h c, b c, "
        "b h t v, b h t v, b h k v -> b h n k v, b h k v, b h t v, b h c"
    ),
...
# custom_vjp (per (chunk_size, use_mask), concrete constexpr)


_DELTA_NET_CHUNK_SANE_TRITON_OP_CACHE = {}


def _get_delta_net_chunk_sane_triton_op(chunk_size, use_mask):
    """返回对特定 (chunk_size, use_mask) 闭包的 custom_vjp 算子。"""
    key = (chunk_size, use_mask)
    if key in _DELTA_NET_CHUNK_SANE_TRITON_OP_CACHE:
        return _DELTA_NET_CHUNK_SANE_TRITON_OP_CACHE[key]

    if use_mask:

        def _fwd_impl(q, k, v, beta, tau, mask, h0):
        ...
        @jax.custom_vjp
        def _op(q, k, v, beta, tau, mask, h0):
            o, ht, _ = _fwd_impl(q, k, v, beta, tau, mask, h0)
            return o, ht

        def _fwd(q, k, v, beta, tau, mask, h0):
            o, ht, res = _fwd_impl(q, k, v, beta, tau, mask, h0)
            return (o, ht), res
        ...
        def _bwd(res, grads):
            ...
            dh, dh0, dv, dtau = _bwd_dhu_sane_with_mask_spmd(
                q2, k2, w, h, v_new, tau, mask, do, dv_local, dht, chunk_size
            )
        ...
        def _fwd_impl(q, k, v, beta, tau, h0):
            q2, inv_norm_q = _l2norm_fwd_spmd(q)
            k2, inv_norm_k = _l2norm_fwd_spmd(k)

            A = _fwd_intra_spmd(k2, beta, chunk_size)
            w, u = _recompute_w_u_spmd(k2, v, beta, A, chunk_size)
            h, v_new, ht = _fwd_h_sane_no_mask_spmd(k2, w, u, tau, h0, chunk_size)
            o = _fwd_o_spmd(q2, k2, v_new, h, chunk_size)
        ...
        @jax.custom_vjp
        def _op(q, k, v, beta, tau, h0):
            o, ht, _ = _fwd_impl(q, k, v, beta, tau, h0)
            return o, ht

        def _fwd(q, k, v, beta, tau, h0):
            o, ht, res = _fwd_impl(q, k, v, beta, tau, h0)
            return (o, ht), res
        ...
        def _bwd(res, grads):
            ...
            dh, dh0, dv, dtau = _bwd_dhu_sane_no_mask_spmd(
                q2, k2, w, h, v_new, tau, do, dv_local, dht, chunk_size
            )
            dq, dk, dw = _bwd_dqk_spmd(q2, k2, v_new, w, h, dh, do, dv, chunk_size)
            dk2, dv2, db = _wy_bwd_spmd(k2, v, beta, A, dw, dv, chunk_size)
            dk = dk + dk2
    ...
    _DELTA_NET_CHUNK_SANE_TRITON_OP_CACHE[key] = _op
    return _op
...
def delta_net_chunk_sane(
    ...
    """DeltaNet chunkwise JAX-Triton SANE 实现（训练前向）。

    接口在 `delta_net_chunk` 基础上增加 SANE 参数 `tau` 与 `mask`。
    ...
    if use_mask:
        ...
        out, final_state = _get_delta_net_chunk_sane_triton_op(chunk_size, True)(
    ...
    out, final_state = _get_delta_net_chunk_sane_triton_op(chunk_size, False)(
        q, k, v, beta, tau, h0
    )
    out = _transpose_back(out)
    out = jnp.asarray(out, dtype)
