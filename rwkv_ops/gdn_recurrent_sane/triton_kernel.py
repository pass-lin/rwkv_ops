"""Gated DeltaNet recurrent SANE Triton kernel（共享实现）。"""
...
@triton.jit
def gated_delta_net_recurrent_sane_fwd_kernel(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 训练前向 Triton kernel。

    每个 program 处理一个 (batch, head) 与 V 方向的一个 block。
    在 chunk 末尾先保存 SANE 之前的 state，再执行 SANE，最后写出 post-SANE state。

    Args:
        q, k: [B, H, T, K]，与输入同 dtype，row-major。
        v, o: [B, H, T, V]，v 与 o 同 dtype，row-major。
        tau: [B, H, T//CHUNK_LEN]，float32，row-major。
        mask: [B, T//CHUNK_LEN]，float32，row-major。USE_MASK=False 时不读。
        kv_mem_out: [B, H, T, V]，float32，每步的 `k_t @ state_{t-1}`。
        state_chkp: [B, H, T//CHUNK_LEN, K, V]，float32，SANE 之前的 state 快照。
        ...
        USE_MASK: tl.constexpr，是否按 mask 选择执行 SANE。
    """
    pid = tl.program_id(0)
    NV = tl.cdiv(V, BV)
    i_v = pid % NV
    ...
    for t in range(T):
        ...
        # 每 chunk 末尾保存 SANE 之前的 state，再执行 SANE。
        if num_chunks > 0 and (t + 1) % CHUNK_LEN == 0:
            ...
            # 读取 tau / mask。
            tau_t = tl.load(tau + bh * num_chunks + chkp_t).to(tl.float32)
            tau_safe = tl.maximum(tau_t, 1e-6)
            if USE_MASK:
                mask_t = tl.load(mask + i_b_i64 * num_chunks + chkp_t).to(tl.float32)
                m = mask_t > 0.0
            else:
                m = True
...
def _bwd_autotune_pre_hook(nargs, reset_only=False):
    ...
    for name in ("dq", "dk", "dv", "dg", "dbeta", "dh0", "dtau"):
        buf = nargs.get(name)
        if buf is None:
            continue
...
@triton.jit
def gated_delta_net_recurrent_sane_bwd_kernel(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 训练反向 Triton kernel。

    每个 program 处理一个 (batch, head) 与 V 方向的一个 block。
    先重新跑一遍前向得到最终状态，再从 T 反向递推到 0；
    在每个 chunk 边界先应用 SANE 导数，再回传 chunk 内梯度。

    Args:
        q, k: [B, H, T, K]，输入同 dtype，row-major。
        v, do: [B, H, T, V]，输入/输出梯度同 dtype，row-major。
        tau: [B, H, T//CHUNK_LEN]，float32。
        mask: [B, T//CHUNK_LEN]，float32。USE_MASK=False 时不读。
    ...
    # 前向重算得到最终状态（不应用 SANE，因为 SANE 导数在循环入口处单独处理）。
    b_h = tl.load(h0 + base_state, mask=mask_h, other=0.0).to(tl.float32)
    for t in range(T):
        t_i64 = t.to(tl.int64)
        p_q = q + base_qk + t_i64 * K_i64
    ...
    for t in range(T - 1, -1, -1):
        ...
        # 到达 chunk 边界时，从快照加载 SANE 之前的 state，并应用 SANE 反向。
        if num_chunks > 0 and tp1 % CHUNK_LEN == 0:
            ...
            # dtau = sum(dS * (tanh(u) - u * sech2))，仅在 mask>0 时计算。
            dtau_local = tl.sum(carry_dS * (tnh - u * sech2))
            dtau_local = tl.where(m, dtau_local, 0.0)
            if NV > 1:
                tl.atomic_add(dtau + bh * num_chunks + chkp_t, dtau_local)
            else:
                tl.store(dtau + bh * num_chunks + chkp_t, dtau_local)

            # 穿过 SANE 的梯度：mask=0 时恒等，mask=1 时乘 sech2。
            grad_factor = tl.where(m, sech2, 1.0)
            carry_dS = carry_dS * grad_factor
...
@triton.jit
def gated_delta_net_recurrent_sane_inference_fwd_kernel(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 推理前向 Triton kernel。
...
@triton.jit
def gated_delta_net_recurrent_sane_single_step_fwd_kernel(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 单步 RNN 前向 Triton kernel。
    ...
    if STORE_FINAL_STATE:
        tau_t = tl.load(tau + bh).to(tl.float32)
        tau_safe = tl.maximum(tau_t, 1e-6)
        do_sane_t = tl.load(do_sane + i_b_i64).to(tl.float32)
        m = do_sane_t > 0.0
        u = b_h / tau_safe
        x2 = 2.0 * u
        tnh = tl.where(
...
@triton.jit
def gated_delta_net_recurrent_sane_fwd_kernel_no_mask(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 训练前向 Triton kernel（无 mask）。

    每个 program 处理一个 (batch, head) 与 V 方向的一个 block。
    与带 mask 版本的区别是 chunk 边界无条件执行 SANE，全程不读取 mask。
    ...
    Args:
        ...
        kv_mem_out: [B, H, T, V]，float32，每步的 `k_t @ state_{t-1}`。
        state_chkp: [B, H, T//CHUNK_LEN, K, V]，float32，SANE 之前的 state 快照。
        inv_norm_q: [B, H, T]，float32，q 的 L2 归一化逆范数。
        inv_norm_k: [B, H, T]，float32，k 的 L2 归一化逆范数。
        g: [B, H, T]，log-space decay，row-major。
        beta: [B, H, T]，已在外部过 sigmoid 的写入强度，row-major。
        h0: [B, H, K, V]，float32，初始 state。
        ht: [B, H, K, V]，float32，最终 state（仅当 STORE_FINAL_STATE=True 时写入）。
        ...
        K, V: tl.constexpr，head 维度大小。
        BK, BV: tl.constexpr，block 大小。
        CHUNK_LEN: tl.constexpr，chunk 长度，默认 16。
    """
    pid = tl.program_id(0)
    NV = tl.cdiv(V, BV)
    i_v = pid % NV
    ...
    for t in range(T):
        ...
        # 每 chunk 末尾保存 SANE 之前的 state，再无条件执行 SANE。
        if num_chunks > 0 and (t + 1) % CHUNK_LEN == 0:
            chkp_t = (t_i64 + 1) // CHUNK_LEN_i64 - 1
            base_chkp = (
                bh * num_chunks * K_i64 * V_i64
...
@triton.jit
def gated_delta_net_recurrent_sane_bwd_kernel_no_mask(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 训练反向 Triton kernel（无 mask）。

    每个 program 处理一个 (batch, head) 与 V 方向的一个 block。
    与带 mask 版本的区别是 chunk 边界无条件计算 dtau 并回传 sech2 梯度，
    全程不读取 mask。
    ...
    for t in range(T - 1, -1, -1):
        ...
        if num_chunks > 0 and tp1 % CHUNK_LEN == 0:
            ...
            # dtau = sum(dS * (tanh(u) - u * sech2))，无 mask 时无条件累加。
            dtau_local = tl.sum(carry_dS * (tnh - u * sech2))
            if NV > 1:
                tl.atomic_add(dtau + bh * num_chunks + chkp_t, dtau_local)
            else:
            ...
            # 穿过 SANE 的梯度：恒乘 sech2。
            carry_dS = carry_dS * sech2
...
@triton.jit
def gated_delta_net_recurrent_sane_inference_fwd_kernel_no_mask(
    q,
    k,
    v,
    g,
    ...
    """Gated DeltaNet recurrent SANE 推理前向 Triton kernel（无 mask）。
