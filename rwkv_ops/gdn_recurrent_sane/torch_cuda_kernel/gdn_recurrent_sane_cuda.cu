// Gated DeltaNet recurrent SANE PyTorch CUDA 训练/推理 kernel。
...
// Gated DeltaNet recurrent SANE 训练前向 kernel。
//
...
//   g_: [B, H, T], float32, row-major。log-space decay。
...
__global__ __launch_bounds__(kBlockThreads) void gdn_recurrent_sane_fwd_kernel(
...
//   g_, beta_: [B, H, T], float32, row-major。
...
//   dg_, dbeta_: [B, H, T], float32, row-major。输出梯度。
...
//   _CHUNK_LEN_: chunk 长度，默认 16，T 必须被其整除。
template <typename ET>
__global__ __launch_bounds__(kBlockThreads) void gdn_recurrent_sane_bwd_kernel(
    int T, int H, float scale, const ET *__restrict__ q_,
    const ET *__restrict__ k_, const ET *__restrict__ v_,
    const float *__restrict__ g_, const float *__restrict__ beta_,
    const float *__restrict__ tau_, const float *__restrict__ mask_,
    ...
    float *__restrict__ dg_, float *__restrict__ dbeta_,
...
// Gated DeltaNet recurrent SANE 推理前向 kernel。
//
...
template <typename ET>
__global__
__launch_bounds__(kBlockThreads) void gdn_recurrent_sane_inference_kernel(
    int T, int H, float scale, const ET *__restrict__ q_,
    const ET *__restrict__ k_, const ET *__restrict__ v_,
    const float *__restrict__ g_, const float *__restrict__ beta_,
    const float *__restrict__ tau_, const float *__restrict__ mask_,
...
// Gated DeltaNet recurrent SANE 单步 RNN 前向 kernel。
//
...
template <typename ET>
__global__
__launch_bounds__(kBlockThreads) void gdn_recurrent_sane_single_step_kernel(
    int H, float scale, const ET *__restrict__ q_, const ET *__restrict__ k_,
    const ET *__restrict__ v_, const float *__restrict__ g_,
...
#pragma unroll
    for (int j = 0; j < _K_; j++) {
      state[j] = h0_[sbase + (int64_t)j * _V_] * exp_g;
...
void cuda_gdn_sane_forward(int B, int T, int H, float scale, const ET *q,
  ...
  gdn_recurrent_sane_fwd_kernel<ET>
      <<<dim3(H, B), kBlockThreads>>>(T, H, scale, q, k, v, g, beta, tau, mask,
...
void cuda_gdn_sane_backward(int B, int T, int H, float scale, const ET *q,
  ...
  gdn_recurrent_sane_bwd_kernel<ET><<<dim3(H, B), kBlockThreads>>>(
      T, H, scale, q, k, v, g, beta, tau, mask, dout, dht, kv_mem, inv_q, inv_k,
      chkp, dq, dk, dv, dg, dbeta, dtau, dh0);
}
...
void cuda_gdn_sane_forward_inference(int B, int T, int H, float scale,
  ...
  gdn_recurrent_sane_inference_kernel<ET><<<dim3(H, B), kBlockThreads>>>(
      T, H, scale, q, k, v, g, beta, tau, mask, h0, o, ht);
}
...
void cuda_gdn_sane_single_step(int B, int H, float scale, const ET *q,
                               ...
                               float *ht) {
  gdn_recurrent_sane_single_step_kernel<ET><<<dim3(H, B), kBlockThreads>>>(
      H, scale, q, k, v, g, beta, tau, do_sane, h0, o, ht);
}

// 显式实例化 bfloat16 与 float32 两个版本，供 .cpp 侧按输入 dtype 分发。
template void cuda_gdn_sane_forward<bf>(int, int, int, float, const bf *,
                                        const bf *, const bf *, const float *,
                                        const float *, const float *,
                                        const float *, const float *, bf *,
                                        float *, float *, float *, float *,
                                        float *);
template void cuda_gdn_sane_forward<float>(int, int, int, float, const float *,
                                           const float *, const float *,
                                           const float *, const float *,
                                           const float *, const float *,
                                           const float *, float *, float *,
                                           float *, float *, float *, float *);
template void cuda_gdn_sane_backward<bf>(
...
template void cuda_gdn_sane_backward<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, const float *,
    float *, float *, float *, float *, float *, float *, float *);
template void cuda_gdn_sane_forward_inference<bf>(
    int, int, int, float, const bf *, const bf *, const bf *, const float *,
    const float *, const float *, const float *, const float *, bf *, float *);
template void cuda_gdn_sane_forward_inference<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, const float *,
    float *, float *);
template void cuda_gdn_sane_single_step<bf>(int, int, float, const bf *,
                                            const bf *, const bf *,
                                            const float *, const float *,
                                            const float *, const float *,
                                            const float *, bf *, float *);
template void cuda_gdn_sane_single_step<float>(int, int, float, const float *,
                                               const float *, const float *,
                                               const float *, const float *,
                                               const float *, const float *,
                                               const float *, float *, float *);
...
//
// 与带 mask 版本数学一致，但 chunk 边界无条件执行 SANE，全程不读取
// mask 指针，也不需要封装层构造全 1 占位张量。
//
// Args:
//   q_, k_: [B, H, T, K], bfloat16 或 float32, row-major。
//   v_, o_: [B, H, T, V], bfloat16 或 float32, row-major。v 为输入，o 为输出。
...
gdn_recurrent_sane_bwd_kernel_no_mask(
    ...
    const ET *__restrict__ k_, const ET *__restrict__ v_,
    const float *__restrict__ g_, const float *__restrict__ beta_,
    const float *__restrict__ tau_, const ET *__restrict__ do_,
...
void cuda_gdn_sane_forward_no_mask(int B, int T, int H, float scale, const ET *q,
                                   ...
                                   float *ht) {
  gdn_recurrent_sane_fwd_kernel_no_mask<ET><<<dim3(H, B), kBlockThreads>>>(
      T, H, scale, q, k, v, g, beta, tau, h0, o, kv_mem, chkp, inv_q, inv_k, ht);
}
...
void cuda_gdn_sane_backward_no_mask(
  ...
  gdn_recurrent_sane_bwd_kernel_no_mask<ET><<<dim3(H, B), kBlockThreads>>>(
      T, H, scale, q, k, v, g, beta, tau, dout, dht, kv_mem, inv_q, inv_k, chkp,
      dq, dk, dv, dg, dbeta, dtau, dh0);
}
...
void cuda_gdn_sane_forward_inference_no_mask(
  ...
  gdn_recurrent_sane_inference_kernel_no_mask<ET>
      <<<dim3(H, B), kBlockThreads>>>(T, H, scale, q, k, v, g, beta, tau, h0,
                                      o, ht);
}

// 无 mask 版本的显式实例化。
template void cuda_gdn_sane_forward_no_mask<bf>(
    int, int, int, float, const bf *, const bf *, const bf *, const float *,
    const float *, const float *, const float *, bf *, float *, float *,
    float *, float *, float *);
template void cuda_gdn_sane_forward_no_mask<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, float *,
    float *, float *, float *, float *, float *);
template void cuda_gdn_sane_backward_no_mask<bf>(
    int, int, int, float, const bf *, const bf *, const bf *, const float *,
    const float *, const float *, const bf *, const float *, const float *,
    const float *, const float *, const float *, float *, float *, float *,
    float *, float *, float *, float *);
template void cuda_gdn_sane_backward_no_mask<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, float *,
    float *, float *, float *, float *, float *, float *);
template void cuda_gdn_sane_forward_inference_no_mask<bf>(
    int, int, int, float, const bf *, const bf *, const bf *, const float *,
    const float *, const float *, const float *, bf *, float *);
template void cuda_gdn_sane_forward_inference_no_mask<float>(
    int, int, int, float, const float *, const float *, const float *,
    const float *, const float *, const float *, const float *, float *,
    float *);
