"""PyTorch 版 Gated DeltaNet recurrent SANE CUDA kernel 封装。"""
...
def _load_ops(K, V, chunk_size):
    ...
    lib_name = f"gdn_recurrent_sane_cuda_{K}_{V}_{chunk_size}"
    flags = [
        "-O3",
        "-Xptxas",
        "-O3",
    ...
    load(
        name=lib_name,
        sources=[
            os.path.join(_CURRENT_DIR, "gdn_recurrent_sane_cuda.cu"),
            os.path.join(_CURRENT_DIR, "gdn_recurrent_sane_op.cpp"),
        ],
...
class _GatedDeltaNetRecurrentSaneCudaFunction(torch.autograd.Function):
    """Gated DeltaNet recurrent SANE 训练 CUDA 封装。"""
    ...
    def forward(
        ...
        ctx.save_for_backward(
            q, k, v, g, beta, tau, mask, kv_mem, state_chkp, inv_norm_q, inv_norm_k
        )
...
class _GatedDeltaNetRecurrentSaneInferenceCudaFunction(torch.autograd.Function):
    """Gated DeltaNet recurrent SANE 推理 CUDA 封装（无反向）。"""
    ...
    def forward(
        ...
        if use_mask:
            ops.forward_inference(
                q, k, v, g, beta, tau, mask, h0, scale, o, final_state
            )
        else:
            # 无 mask 路径：chunk 边界无条件 SANE，不传占位 mask。
            ops.forward_inference_no_mask(
                q, k, v, g, beta, tau, h0, scale, o, final_state
            )
...
class _GatedDeltaNetRecurrentSaneSingleStepCudaFunction(torch.autograd.Function):
    """Gated DeltaNet recurrent SANE 单步 RNN CUDA 封装（无反向）。"""
    ...
    def forward(
        ...
        if not head_first:
            raise NotImplementedError(
                "gated_delta_net_recurrent_sane_single_step currently only "
        ...
        ops.single_step(q, k, v, g, beta, tau, do_sane, h0, scale, o, next_state)
...
def gated_delta_net_recurrent_sane(
    ...
    """Gated DeltaNet recurrent SANE 训练算子（PyTorch CUDA 实现）。
    ...
    if q.device.type != "cuda":
        from ..native_keras_op import (
            gated_delta_net_recurrent_sane as native_op,
        )
    ...
    return _GatedDeltaNetRecurrentSaneCudaFunction.apply(
        q,
        k,
        v,
        g,
...
def gated_delta_net_recurrent_sane_inference(
    ...
    """Gated DeltaNet recurrent SANE 推理算子（PyTorch CUDA 实现，无梯度）。
    ...
    if q.device.type != "cuda":
        raise NotImplementedError(
            "Gated DeltaNet recurrent SANE CUDA inference kernel only supports "
            "CUDA devices."
        )

    return _GatedDeltaNetRecurrentSaneInferenceCudaFunction.apply(
        q,
        k,
        v,
        g,
...
def gated_delta_net_recurrent_sane_single_step(
    ...
    """Gated DeltaNet recurrent SANE 单步 RNN 算子（PyTorch CUDA 实现）。
    ...
    if q.device.type != "cuda":
        raise NotImplementedError(
            "Gated DeltaNet recurrent SANE CUDA single step kernel only "
            "supports CUDA devices."
        )

    return _GatedDeltaNetRecurrentSaneSingleStepCudaFunction.apply(
        q,
        k,
        v,
        g,
