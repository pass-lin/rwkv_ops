// Gated DeltaNet recurrent SANE JAX FFI CUDA 训练/推理 kernel。
...
// S_{t+1}，避免长序列反向递推中反复除以 exp(g) 导致的数值爆炸，同时应用
...
static ffi::Error GdnSaneFwdHost(
  ...
  gdn_recurrent_sane_fwd_kernel<ET><<<dim3(H, B), kBlockThreads, 0, stream>>>(
      ...
      beta.typed_data(), tau.typed_data(), mask.typed_data(), h0.typed_data(),
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("gdn_recurrent_sane_fwd error: ") +
                                cudaGetErrorString(err));
  return ffi::Error::Success();
}

template <typename ET, ffi::DataType DT>
static ffi::Error GdnSaneBwdHost(
    cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
    ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> g, ffi::Buffer<ffi::F32> beta,
  ...
  gdn_recurrent_sane_bwd_kernel<ET><<<dim3(H, B), kBlockThreads, 0, stream>>>(
      T, H, scale, reinterpret_cast<const ET *>(q.typed_data()),
      reinterpret_cast<const ET *>(k.typed_data()),
      reinterpret_cast<const ET *>(v.typed_data()), g.typed_data(),
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("gdn_recurrent_sane_bwd error: ") +
                                cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
template <typename ET, ffi::DataType DT>
static ffi::Error
GdnSaneInferenceHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
  ...
  gdn_recurrent_sane_inference_kernel<ET>
      <<<dim3(H, B), kBlockThreads, 0, stream>>>(
          T, H, scale, reinterpret_cast<const ET *>(q.typed_data()),
          reinterpret_cast<const ET *>(k.typed_data()),
          reinterpret_cast<const ET *>(v.typed_data()), g.typed_data(),
          beta.typed_data(), tau.typed_data(), mask.typed_data(),
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("gdn_recurrent_sane_inference error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
template <typename ET, ffi::DataType DT>
static ffi::Error
GdnSaneSingleStepHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
                      ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> g,
                      ffi::Buffer<ffi::F32> beta, ffi::Buffer<ffi::F32> tau,
  ...
  gdn_recurrent_sane_single_step_kernel<ET>
      <<<dim3(H, B), kBlockThreads, 0, stream>>>(
          H, scale, reinterpret_cast<const ET *>(q.typed_data()),
          reinterpret_cast<const ET *>(k.typed_data()),
          reinterpret_cast<const ET *>(v.typed_data()), g.typed_data(),
          beta.typed_data(), tau.typed_data(), do_sane.typed_data(),
          h0.typed_data(), reinterpret_cast<ET *>(o->typed_data()),
          ht->typed_data());
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("gdn_recurrent_sane_single_step error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
#define GDN_SANE_DEFINE_FWD_HANDLER(SYMBOL, ET, DT)                            \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnSaneFwdHost<ET, DT>),              \
                                ffi::Ffi::Bind()                               \
...
#define GDN_SANE_DEFINE_BWD_HANDLER(SYMBOL, ET, DT)                            \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnSaneBwdHost<ET, DT>),              \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
#define GDN_SANE_DEFINE_INF_HANDLER(SYMBOL, ET, DT)                            \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnSaneInferenceHost<ET, DT>),        \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
#define GDN_SANE_DEFINE_SINGLE_HANDLER(SYMBOL, ET, DT)                         \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(                                               \
      SYMBOL, (GdnSaneSingleStepHost<ET, DT>),                                 \
      ffi::Ffi::Bind()                                                         \
          .Ctx<ffi::PlatformStream<cudaStream_t>>()                            \
          .Arg<ffi::Buffer<DT>>()       /* q */                                \
          .Arg<ffi::Buffer<DT>>()       /* k */                                \
...
GDN_SANE_DEFINE_FWD_HANDLER(GdnRecurrentSaneFwdBf16, bf, ffi::BF16);
GDN_SANE_DEFINE_BWD_HANDLER(GdnRecurrentSaneBwdBf16, bf, ffi::BF16);
GDN_SANE_DEFINE_INF_HANDLER(GdnRecurrentSaneInferenceBf16, bf, ffi::BF16);
GDN_SANE_DEFINE_SINGLE_HANDLER(GdnRecurrentSaneSingleStepBf16, bf, ffi::BF16);

GDN_SANE_DEFINE_FWD_HANDLER(GdnRecurrentSaneFwdF32, float, ffi::F32);
