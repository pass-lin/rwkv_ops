...
def _make_gdn_recurrent_sane_fwd_kernel_no_mask(chunk_size: int):
    """构造 Gated DeltaNet recurrent SANE 训练前向 Pallas kernel（无 mask）。
    ...
    def _gdn_recurrent_sane_fwd_kernel_no_mask(
        q_ref,
        k_ref,
        v_ref,
        g_ref,
    ...
    return _gdn_recurrent_sane_fwd_kernel_no_mask
...
def _make_gdn_recurrent_sane_bwd_kernel_no_mask(chunk_size: int):
    """构造 Gated DeltaNet recurrent SANE 训练反向 Pallas kernel（无 mask）。

    返回的 kernel 在 grid (B, N) 上运行，每个 program 处理一个 (batch, head)。
    在每个 chunk 边界先应用 SANE 反向（无条件累加 dtau），再回传 chunk 内梯度。
    """

    def _gdn_recurrent_sane_bwd_kernel_no_mask(
        q_ref,
        k_ref,
        v_ref,
        g_ref,
    ...
    return _gdn_recurrent_sane_bwd_kernel_no_mask
...
def _make_gdn_recurrent_sane_inf_kernel_no_mask(chunk_size: int):
    """构造 Gated DeltaNet recurrent SANE 推理前向 Pallas kernel（无 mask）。

    与训练前向数学一致，但不做反向保存；chunk 边界无条件 SANE，支持任意 T。
    """

    def _gdn_recurrent_sane_inf_kernel_no_mask(
        q_ref,
        k_ref,
        v_ref,
        g_ref,
    ...
    return _gdn_recurrent_sane_inf_kernel_no_mask
...
def _gdn_recurrent_sane_fwd_pallas_call_no_mask(
    q, k, v, g, beta, tau, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"gdn_recurrent_sane_fwd_no_mask_{chunk_size}",
        _make_gdn_recurrent_sane_fwd_kernel_no_mask(chunk_size),
        _fwd_out_shape(q, v, chunk_size),
        (B, N),
        (q, k, v, g, beta, tau, h0),
    )
...
def _gdn_recurrent_sane_fwd_warmup_no_mask(q, k, v, g, beta, tau, h0, chunk_size: int):
    B, N, T, K = q.shape
    ensure_config(
        f"gdn_recurrent_sane_fwd_no_mask_{chunk_size}",
        _make_gdn_recurrent_sane_fwd_kernel_no_mask(chunk_size),
...
def _gdn_recurrent_sane_fwd_spmd_no_mask(q, k, v, g, beta, tau, h0, chunk_size: int):
    return _gdn_recurrent_sane_fwd_pallas_call_no_mask(
...
_gdn_recurrent_sane_fwd_spmd_no_mask.def_partition(
    ...
    partition=create_partition(_gdn_recurrent_sane_fwd_pallas_call_no_mask),
)
...
def _gdn_recurrent_sane_bwd_pallas_call_no_mask(
    q,
    k,
    v,
    g,
    ...
    B, N, T, K = q.shape
    return launch(
        f"gdn_recurrent_sane_bwd_no_mask_{chunk_size}",
        _make_gdn_recurrent_sane_bwd_kernel_no_mask(chunk_size),
        _bwd_out_shape(q, v, chunk_size),
        (B, N),
        (q, k, v, g, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, chkp, h0, tau),
    )


def _gdn_recurrent_sane_bwd_warmup_no_mask(
    q,
    k,
    v,
    g,
...
def _gdn_recurrent_sane_bwd_spmd_no_mask(
    ...
    return _gdn_recurrent_sane_bwd_pallas_call_no_mask(
        q,
        k,
        v,
        g,
...
_gdn_recurrent_sane_bwd_spmd_no_mask.def_partition(
    ...
    partition=create_partition(_gdn_recurrent_sane_bwd_pallas_call_no_mask),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(7,))
def _gdn_recurrent_sane_train_no_mask(q, k, v, g, beta, tau, h0, chunk_size: int):
    _gdn_recurrent_sane_fwd_warmup_no_mask(q, k, v, g, beta, tau, h0, chunk_size)
...
def _gdn_train_bwd_no_mask(chunk_size: int, res, grads):
    ...
    _gdn_recurrent_sane_bwd_warmup_no_mask(
        q,
        k,
        v,
        g,
...
def _gdn_recurrent_sane_inf_pallas_call_no_mask(
    q, k, v, g, beta, tau, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"gdn_recurrent_sane_inf_no_mask_{chunk_size}",
        _make_gdn_recurrent_sane_inf_kernel_no_mask(chunk_size),
        _inf_out_shape(q, v),
        (B, N),
        (q, k, v, g, beta, tau, h0),
    )
...
def _gdn_recurrent_sane_inf_warmup_no_mask(q, k, v, g, beta, tau, h0, chunk_size: int):
    B, N, T, K = q.shape
    ensure_config(
        f"gdn_recurrent_sane_inf_no_mask_{chunk_size}",
        _make_gdn_recurrent_sane_inf_kernel_no_mask(chunk_size),
...
def _gdn_recurrent_sane_inf_spmd_no_mask(q, k, v, g, beta, tau, h0, chunk_size: int):
    return _gdn_recurrent_sane_inf_pallas_call_no_mask(
        q, k, v, g, beta, tau, h0, chunk_size
    )
...
_gdn_recurrent_sane_inf_spmd_no_mask.def_partition(
    ...
    partition=create_partition(_gdn_recurrent_sane_inf_pallas_call_no_mask),
)
...
def gated_delta_net_recurrent_sane_inference(
    ...
    if use_mask:
        ...
        # 无 mask 路径：chunk 边界无条件执行 SANE，不构造占位 mask。
        _gdn_recurrent_sane_inf_warmup_no_mask(q, k, v, g, beta, tau, h0, chunk_size)
