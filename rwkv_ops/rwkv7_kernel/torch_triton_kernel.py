...
def _make_triton_op(chunk_size: int):
    ...
    return TritonWindBackstepping


def _make_triton_op_with_mask(chunk_size: int):
    ...
    return TritonWindBacksteppingWithMask
...
def generalized_delta_rule(
    ...
    Raises:
        ...
        ValueError: T 不被 chunk_size 整除，或 H 不等于 64。
    """
    ...
    Op = (
        _make_triton_op_with_mask(chunk_size)
        if mask is not None
        else _make_triton_op(chunk_size)
    )

    if mask is None:
        out, state = Op.apply(w, r, k, v, a, b, initial_state)
    else:
        if not mask.is_cuda:
            mask = mask.to(r.device)
        out, state = Op.apply(w, r, k, v, a, b, mask, initial_state)
