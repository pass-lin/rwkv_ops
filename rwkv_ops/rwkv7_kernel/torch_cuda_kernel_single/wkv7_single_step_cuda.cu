...
__launch_bounds__(C, 2) __global__
    void forward_single_step_kernel(int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_,
                                    F_ b_, float *h0_, bf *y_, float *h1_) {
...
#pragma unroll
  ...
  __shared__ float shared_a[C], shared_b[C], shared_w[C], shared_k[C],
      shared_q[C];
...
#pragma unroll
  for (int j = 0; j < C; j++) {
    state_row[j] =
        state_row[j] * shared_w[j] + sa * shared_b[j] + shared_k[j] * v_val;
...
// 编译期宏:
//   _C_: head_size。
void cuda_forward_single_step(int B, int H, bf *w, bf *q, bf *k, bf *v, bf *a,
                              bf *b, float *h0, bf *y, float *h1) {
  dim3 blocks(H, B);
  dim3 threads(_C_);

  forward_single_step_kernel<_C_>
      <<<blocks, threads>>>(H, w, q, k, v, a, b, h0, y, h1);
}