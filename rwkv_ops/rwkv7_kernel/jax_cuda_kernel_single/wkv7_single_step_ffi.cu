...
__launch_bounds__(C, 2) __global__
    void forward_kernel_single_step(int B, int H, F_ w_, F_ q_, F_ k_, F_ v_,
                                    F_ a_, F_ b_, bf *y_, float *s_,
                                    float *h0_) {
...
template <int C>
__launch_bounds__(C, 2) __global__
    void backward_kernel_single_step(int B, int H, F_ w_, F_ q_, F_ k_, F_ v_,
                                     F_ dy_, float *s_, float *dht_, bf *dw_,
...
#pragma unroll
  for (int j = 0; j < C; ++j)
    stateT[j] = s_[s_base + j];

  float dq_val = 0.f, dw_val = 0.f, dk_val = 0.f, dv_val = 0.f, da_val = 0.f,
        db_val = 0.f;
...
static ffi::Error
WKV7SingleStepFwdHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> w,
  ...
  forward_kernel_single_step<_C_>
      <<<grid, block, 0, stream>>>(B, H, reinterpret_cast<bf *>(w.typed_data()),
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("CUDA forward_kernel_single_step error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}

// 注册 JAX FFI 符号。
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7SingleStepFwd, WKV7SingleStepFwdHost,
                              ffi::Ffi::Bind()
