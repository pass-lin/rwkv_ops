...
_SO_PATH = _CURRENT_DIR / "build/wkv7.so"  # 同目录产出 .so
...
def _ensure_compiled() -> pathlib.Path:
    ...
    cmake_args = [
        ...
        "-DCMAKE_INSTALL_PREFIX=" + str(_CURRENT_DIR),  # 安装到当前目录
    ]
    ...
    if not _SO_PATH.exists():
        raise RuntimeError("Compilation failed – librwkv7_jax.so not found.")
