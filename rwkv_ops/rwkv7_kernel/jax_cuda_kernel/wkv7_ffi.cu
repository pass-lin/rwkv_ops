...
//   s_: [B, H, C, K, K]，float32，row-major。每 chunk 结束时的 state
//   checkpoint。
//
// 编译期宏:
//   _C_: head_size。
...
__launch_bounds__(C, 2) __global__
    void backward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_,
                         F_ dy_, float *s_, float *sa_, float *dht_,
                         float *dh0_, bf *dw_, bf *dq_, bf *dk_, bf *dv_,
                         bf *da_, bf *db_) {
  int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
...
static ffi::Error
WKV7FwdHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> w,
            ffi::Buffer<ffi::BF16> q, ffi::Buffer<ffi::BF16> k,
            ffi::Buffer<ffi::BF16> v, ffi::Buffer<ffi::BF16> a,
            ffi::Buffer<ffi::BF16> b, ffi::Buffer<ffi::F32> h0,
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("CUDA forward_kernel error: ") +
...
static ffi::Error
WKV7BwdHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> w,
            ffi::Buffer<ffi::BF16> q, ffi::Buffer<ffi::BF16> k,
            ffi::Buffer<ffi::BF16> v, ffi::Buffer<ffi::BF16> a,
            ffi::Buffer<ffi::BF16> b, ffi::Buffer<ffi::BF16> dy,
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("CUDA backward_kernel error: ") +
                                cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
static ffi::Error
WKV7InferenceHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> w,
                  ffi::Buffer<ffi::BF16> q, ffi::Buffer<ffi::BF16> k,
                  ffi::Buffer<ffi::BF16> v, ffi::Buffer<ffi::BF16> a,
                  ffi::Buffer<ffi::BF16> b, ffi::Buffer<ffi::F32> h0,
  ...
  forward_inference_kernel<_C_>
      <<<grid, block, 0, stream>>>(T, H, reinterpret_cast<bf *>(w.typed_data()),
                                   reinterpret_cast<bf *>(q.typed_data()),
                                   reinterpret_cast<bf *>(k.typed_data()),
                                   reinterpret_cast<bf *>(v.typed_data()),
                                   reinterpret_cast<bf *>(a.typed_data()),
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("CUDA forward_inference_kernel error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}

//  FFI 注册（无 Mask）
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7Fwd, WKV7FwdHost,
                              ffi::Ffi::Bind()
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7Bwd, WKV7BwdHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // w
                                  .Arg<ffi::Buffer<ffi::BF16>>() // q
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7Inference, WKV7InferenceHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // w
                                  .Arg<ffi::Buffer<ffi::BF16>>() // q
...
//
// 每个 block 处理一个 (batch, head)，顺序扫描 T 步，按 mask
// 选择更新或冻结状态。
//
// Args:
//   w_, q_, k_, v_, a_, b_: [B, T, H, K]，bfloat16，row-major。
...
//
// 每个 block 处理一个 (batch, head)，逆序扫描 T 步，按 mask
// 拆分参数梯度与状态梯度。
//
// Args:
//   w_, q_, k_, v_, a_, b_, dy_: [B, T, H, K]，bfloat16，row-major。
...
__launch_bounds__(C, 2) __global__
    void backward_kernel_with_mask(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_,
                                   F_ a_, F_ b_, const bf *__restrict__ mask_,
                                   F_ dy_, float *s_, float *sa_, float *dht_,
                                   float *dh0_, bf *dw_, bf *dq_, bf *dk_,
...
#pragma unroll
    for (int j = 0; j < C; ++j) {
      ...
      // m=1: 仅 trans（标准回传）；m=0: trans + penetration（当前回传 +
      // 未来穿透）
...
static ffi::Error
WKV7FwdWithMaskHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> w,
                    ffi::Buffer<ffi::BF16> q, ffi::Buffer<ffi::BF16> k,
                    ffi::Buffer<ffi::BF16> v, ffi::Buffer<ffi::BF16> a,
                    ffi::Buffer<ffi::BF16> b, ffi::Buffer<ffi::BF16> mask,
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("CUDA forward_kernel_with_mask error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
static ffi::Error WKV7BwdWithMaskHost(
    ...
    ffi::Buffer<ffi::BF16> mask, ffi::Buffer<ffi::BF16> dy,
    ffi::Buffer<ffi::F32> s, ffi::Buffer<ffi::F32> sa,
    ffi::Buffer<ffi::F32> dht, ffi::ResultBuffer<ffi::F32> dh0,
    ffi::ResultBuffer<ffi::BF16> dw, ffi::ResultBuffer<ffi::BF16> dq,
    ffi::ResultBuffer<ffi::BF16> dk, ffi::ResultBuffer<ffi::BF16> dv,
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("CUDA backward_kernel_with_mask error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
static ffi::Error WKV7InferenceWithMaskHost(
    ...
    ffi::Buffer<ffi::BF16> k, ffi::Buffer<ffi::BF16> v,
    ffi::Buffer<ffi::BF16> a, ffi::Buffer<ffi::BF16> b,
    ffi::Buffer<ffi::BF16> mask, ffi::Buffer<ffi::F32> h0,
    ffi::ResultBuffer<ffi::BF16> y, ffi::ResultBuffer<ffi::F32> s) {
  auto dims = w.dimensions();
  int B = dims[0], T = dims[1], H = dims[2];
  constexpr int C = _C_;
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("CUDA forward_inference_kernel_with_mask error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}

//  FFI 注册（带 Mask）
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7FwdWithMask, WKV7FwdWithMaskHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // w
                                  .Arg<ffi::Buffer<ffi::BF16>>() // q
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7BwdWithMask, WKV7BwdWithMaskHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // w
                                  .Arg<ffi::Buffer<ffi::BF16>>() // q
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv7InferenceWithMask, WKV7InferenceWithMaskHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // w
                                  .Arg<ffi::Buffer<ffi::BF16>>() // q
