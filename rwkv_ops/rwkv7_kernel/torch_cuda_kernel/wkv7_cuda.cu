...
//   s_:  [B, T//16, H, K, K], float32, row-major。每个 chunk 末的 state
//   checkpoint。 sa_: [B, T, H, K], float32, row-major。中间量 sa 供反向使用。
...
__launch_bounds__(C, 2) __global__
    void forward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_,
...
//
// Args:
//   w_, q_, k_, v_, a_, b_, dy_: [B, T, H, K], bfloat16, row-major。dy
//   为输出梯度。 s_:  [B, T//16, H, K, K], float32, row-major。前向保存的 state
...
//   dw_, dq_, dk_, dv_, da_, db_: [B, T, H, K], bfloat16,
//   row-major。各输入梯度。
//
// Grid / Block:
//   grid  (H, B)，每个 block 对应一个 (head, batch)。
...
template <int C>
__launch_bounds__(C, 2) __global__
    void backward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_,
                         F_ dy_, float *__restrict__ s_,
                         float *__restrict__ sa_, float *__restrict__ dht_,
                         float *__restrict__ dh0_, bf *dw_, bf *dq_, bf *dk_,
                         bf *dv_, bf *da_, bf *db_) {
...
// RWKV-7 前向推理 kernel。
//
// 每个 block 处理一个 (batch, head)，按时间步顺序扫描 T 步，只输出 y 与最终
// state。
//
// Args:
//   w_, q_, k_, v_, a_, b_: [B, T, H, K], bfloat16, row-major。
...
template <int C>
__launch_bounds__(C, 2) __global__
    void forward_inference_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_,
                                  F_ a_, F_ b_, bf *y_, float *s_, float *h0_) {
  int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
  float state[C] = {0};
  __shared__ float q[C], k[C], w[C], a[C], b[C];
  int64_t h0_base = ((int64_t)bb * H + hh) * C * C + i * C;
...
template <int C>
__launch_bounds__(C, 2) __global__
    void forward_kernel_with_mask(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_,
                                  F_ a_, F_ b_, bf *y_, float *s_, float *sa_,
                                  float *h0_, const bf *__restrict__ mask_) {
  int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
  float state[C] = {0};
  __shared__ float q[C], k[C], w[C], a[C], b[C];
...
//
// 每个 block 处理一个 (batch, head)，从 T-1 倒序扫描到 0，根据 mask
// 计算各输入梯度。
//
// Args:
//   w_, q_, k_, v_, a_, b_: [B, T, H, K], bfloat16, row-major。
...
template <int C>
__launch_bounds__(C, 2) __global__
    void backward_kernel_with_mask(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_,
                                   F_ a_, F_ b_, const bf *__restrict__ mask_,
                                   F_ dy_, float *__restrict__ s_,
                                   float *__restrict__ sa_,
                                   float *__restrict__ dht_,
                                   float *__restrict__ dh0_, bf *dw_, bf *dq_,
...
template <int C>
__launch_bounds__(C, 2) __global__
    void forward_inference_kernel_with_mask(int T, int H, F_ w_, F_ q_, F_ k_,
                                            F_ v_, F_ a_, F_ b_, bf *y_,
                                            float *s_, float *h0_,
...
void cuda_forward(int B, int T, int H, bf *w, bf *q, bf *k, bf *v, bf *z, bf *a,
  ...
  forward_kernel<_C_>
      <<<dim3(H, B), dim3(_C_)>>>(T, H, w, q, k, v, z, a, y, s, sa, h0);
}
...
//   dw, dq, dk, dv, dz, da: [B, T, H, K], bfloat16；dz/da 对应 kernel 的
//   da_/db_。
//
// 编译期宏:
//   _C_: head_size；_CHUNK_LEN_: chunk 长度，固定 16。
void cuda_backward(int B, int T, int H, bf *w, bf *q, bf *k, bf *v, bf *z,
  ...
  backward_kernel<_C_><<<dim3(H, B), dim3(_C_)>>>(
      T, H, w, q, k, v, z, a, dy, s, sa, dht, dh0, dw, dq, dk, dv, dz, da);
}
...
void cuda_forward_inference(int B, int T, int H, bf *w, bf *q, bf *k, bf *v,
  ...
  forward_inference_kernel<_C_>
      <<<dim3(H, B), dim3(_C_)>>>(T, H, w, q, k, v, a, b, y, s, h0);
}
...
void cuda_forward_with_mask(int B, int T, int H, bf *w, bf *q, bf *k, bf *v,
                            ...
                            bf *mask) {
  forward_kernel_with_mask<_C_>
      <<<dim3(H, B), dim3(_C_)>>>(T, H, w, q, k, v, a, b, y, s, sa, h0, mask);
}
...
void cuda_backward_with_mask(int B, int T, int H, bf *w, bf *q, bf *k, bf *v,
  ...
  assert(T % _CHUNK_LEN_ == 0);
  backward_kernel_with_mask<_C_>
      <<<dim3(H, B), dim3(_C_)>>>(T, H, w, q, k, v, a, b, mask, dy, s, sa, dht,
                                  dh0, dw, dq, dk, dv, da, db);
}
...
void cuda_forward_inference_with_mask(int B, int T, int H, bf *w, bf *q, bf *k,
                                      ...
                                      float *h0, bf *mask) {
  forward_inference_kernel_with_mask<_C_>
      <<<dim3(H, B), dim3(_C_)>>>(T, H, w, q, k, v, a, b, y, s, h0, mask);
}