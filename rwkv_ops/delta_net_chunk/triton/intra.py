"""DeltaNet chunkwise intra-chunk 下三角 solve Triton kernel。

计算 `A = (I + L)^{-1}`，其中 `L_ij = beta_i * k_i^T k_j`（严格下三角）。
...
@triton.jit
def _delta_net_chunk_fwd_intra_kernel(
    k_ptr,
    beta_ptr,
    B,
    H,
    ...
    # L = beta_i * k_i^T k_j，严格下三角
    b_L = b_kkt * b_beta[:, None]
...
def delta_net_chunk_fwd_intra(k, beta, chunk_size=64):
    ...
    grid = (B * H * N,)
    _delta_net_chunk_fwd_intra_kernel[grid](
        k,
        beta,
        B,
        H,
