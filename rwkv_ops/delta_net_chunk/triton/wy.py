"""DeltaNet chunkwise WY 表示 Triton kernel。
...
@triton.jit
def _delta_net_chunk_recompute_w_u_fwd_kernel(
    k_ptr,
    v_ptr,
    beta_ptr,
    A_ptr,
...
def delta_net_chunk_recompute_w_u(k, v, beta, A, chunk_size=64):
    """重算 WY 表示 `w, u` 封装。
    ...
    grid = (B * H * N,)
    _delta_net_chunk_recompute_w_u_fwd_kernel[grid](
        k,
        v,
        beta,
        A,
