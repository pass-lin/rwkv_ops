"""DeltaNet chunkwise 局部 dv Triton kernel。

从 `q, k, do` 计算每个 chunk 内的局部 `dv`：
  `dv_local = causal(k @ q^T) @ do * scale`

chunk 间的状态梯度项 `k @ dh` 在 `chunk_bwd_dhu` 中累加。
...
@triton.jit
def _delta_net_chunk_bwd_dv_local_kernel(
    q_ptr,
    k_ptr,
    do_ptr,
    B,
    ...
    # 对 A 项应用 causal mask（包含对角线）
    scale_f32 = tl.cast(scale, tl.float32)
    mask_upper = o_c[:, None] <= o_c[None, :]
    b_A = tl.where(mask_upper & m_c[:, None] & m_c[None, :], b_A * scale_f32, 0.0)
...
def delta_net_chunk_bwd_dv_local(q, k, do, scale, chunk_size=64):
    """局部 dv backward 封装。
    ...
    _delta_net_chunk_bwd_dv_local_kernel[grid](
        q,
        k,
        do,
        B,
