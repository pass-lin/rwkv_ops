"""DeltaNet chunkwise chunk 间状态递推 Triton kernel。
...
@triton.jit
def _delta_net_chunk_fwd_h_kernel(
    k_ptr,
    w_ptr,
    u_ptr,
    h0_ptr,
    ...
    for i_t in range(N):
        ...
        # 更新状态：h = h + k^T @ v_new
        b_h += tl.dot(tl.trans(b_k).to(b_v_new.dtype), b_v_new, allow_tf32=False)
...
def delta_net_chunk_fwd_h(
    k, w, u, initial_state=None, output_final_state=False, chunk_size=64
):
    """chunk 间状态递推封装。
    ...
    _delta_net_chunk_fwd_h_kernel[grid](
        k,
        w,
        u,
        h0,
