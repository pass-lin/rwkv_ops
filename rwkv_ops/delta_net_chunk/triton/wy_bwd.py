"""DeltaNet chunkwise WY 表示反向 Triton kernel。
...
@triton.jit
def _delta_net_chunk_prepare_wy_repr_bwd_kernel(
    k_ptr,
    v_ptr,
    beta_ptr,
    A_ptr,
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_dA += tl.dot(b_dw, tl.trans(b_kb), allow_tf32=False)
        b_dkb = tl.dot(b_A, b_dw, allow_tf32=False)
...
def delta_net_chunk_prepare_wy_repr_bwd(k, v, beta, A, dw, du, chunk_size=64):
    """WY 表示反向封装。
    ...
    grid = (B * H * (T // C),)
    _delta_net_chunk_prepare_wy_repr_bwd_kernel[grid](
        k,
        v,
        beta,
        A,
