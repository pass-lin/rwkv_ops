"""DeltaNet chunkwise 最终输出 Triton kernel。

计算 `o = scale * (q @ h + causal(q @ k^T) @ v_new)`。
...
@triton.jit
def _delta_net_chunk_fwd_o_kernel(
    q_ptr,
    k_ptr,
    v_new_ptr,
    h_ptr,
    ...
    # 对 intra 项应用 causal mask（包含对角线）
    mask_lower = o_c[:, None] >= o_c[None, :]
    b_A = tl.where(mask_lower & m_c[:, None] & m_c[None, :], b_A, 0.0)
...
def delta_net_chunk_fwd_o(q, k, v_new, h, chunk_size=64):
    """chunk 最终输出封装。
    ...
    _delta_net_chunk_fwd_o_kernel[grid](
        q,
        k,
        v_new,
        h,
