...
def delta_net_chunk(
    ...
    """DeltaNet chunkwise 原生实现（数值参考）。

    把输入序列按 chunk_size 分块，通过 WY 表示把串行 recurrent 计算并行化。
    无 decay 门，chunk 内衰减矩阵退化为下三角全 1 矩阵；chunk 之间按
    recurrent 方式传递 state。
