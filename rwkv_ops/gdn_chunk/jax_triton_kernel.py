...
def _fwd_h_call(k, w, u, g, h0, chunk_size):
    B, H, T, K = k.shape
    V = u.shape[-1]
    C = chunk_size
    N = T // C
    ...
    h, v_new, ht = jt.triton_call(
        ...
        K,
        V,
        kernel=_gdn_chunk_fwd_h_kernel,
        out_shape=out_shapes,
        grid=grid,
        C=C,
        BK=BK,
...
_fwd_h_spmd = custom_partitioning(_fwd_h_call, static_argnums=(5,))
_fwd_h_spmd.def_partition(
    ...
    sharding_rule="b h t k, b h t k, b h t v, b h t, b h k v -> b h n k v, b h t v, b h k v",
...
def _bwd_dhu_call(q, k, w, g, do, dv_local, dht, chunk_size):
    B, H, T, K = q.shape
    V = do.shape[-1]
    C = chunk_size
    N = T // C
...
_bwd_dhu_spmd = custom_partitioning(_bwd_dhu_call, static_argnums=(7,))
_bwd_dhu_spmd.def_partition(
    infer_sharding_from_operands=lambda arg_shapes, arg_shardings: (
        _for_h(arg_shardings[0]),
        _for_final(arg_shardings[0]),
        _like_q(arg_shardings[4]),
    ),
    sharding_rule="b h t k, b h t k, b h t k, b h t, b h t v, b h t v, b h k v -> b h n k v, b h k v, b h t v",
...
_GDN_CHUNK_TRITON_OP_CACHE = {}


def _get_gdn_chunk_triton_op(chunk_size):
    """返回对特定 chunk_size 闭包的 custom_vjp 算子。"""
    if chunk_size in _GDN_CHUNK_TRITON_OP_CACHE:
        return _GDN_CHUNK_TRITON_OP_CACHE[chunk_size]

    def _fwd_impl(q, k, v, g, beta, h0):
    ...
    @jax.custom_vjp
    def _op(q, k, v, g, beta, h0):
        o, ht, _ = _fwd_impl(q, k, v, g, beta, h0)
        return o, ht

    def _fwd(q, k, v, g, beta, h0):
        o, ht, res = _fwd_impl(q, k, v, g, beta, h0)
        return (o, ht), res
    ...
    def _bwd(res, grads):
        ...
        dh, dh0, dv = _bwd_dhu_spmd(q2, k2, w, g_cum, do, dv_local, dht, chunk_size)
...
def gated_delta_net_chunk(
    ...
    """Gated DeltaNet chunkwise JAX-Triton 实现（训练前向）。
    ...
    _op = _get_gdn_chunk_triton_op(chunk_size)
    out, final_state = _op(q, k, v, g, beta, h0)
