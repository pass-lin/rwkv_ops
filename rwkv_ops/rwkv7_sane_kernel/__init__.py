...
def get_generalized_delta_rule_sane(HEAD_SIZE=64, KERNEL_TYPE="cuda"):
    ...
    if keras.config.backend() == "jax":
        ...
        if torch.cuda.is_available():
            if KERNEL_TYPE == "cuda":
                ...
                return get_torch_generalized_delta_rule_sane(HEAD_SIZE)
            elif KERNEL_TYPE == "triton":
...
def get_rnn_generalized_delta_rule_sane(HEAD_SIZE=64, KERNEL_TYPE="cuda"):
    ...
    if KERNEL_TYPE == "cuda":
        if keras.config.backend() == "jax":
            ...
            if jax.devices()[0].platform == "gpu":
                ...
                return get_jax_generalized_delta_rule_sane_single_step(HEAD_SIZE)
        elif keras.config.backend() == "torch":
            import torch
            ...
            if torch.cuda.is_available():
                ...
                return get_torch_generalized_delta_rule_sane_single_step(HEAD_SIZE)
