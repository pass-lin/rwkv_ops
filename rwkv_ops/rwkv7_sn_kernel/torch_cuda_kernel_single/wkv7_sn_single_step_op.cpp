...
void forward_single_step_sn(torch::Tensor w, torch::Tensor q, torch::Tensor k,
                            torch::Tensor v, torch::Tensor a, torch::Tensor b,
                            torch::Tensor tau, torch::Tensor do_sn,
  ...
  TORCH_CHECK(do_sn.dtype() == torch::kInt8, "do_sn must be int8");
  ...
  cuda_forward_single_step_sn(
      ...
      do_sn.data_ptr<int8_t>(), h0.data_ptr<float>(),
      reinterpret_cast<bf *>(y.data_ptr()), h1.data_ptr<float>());
}

TORCH_LIBRARY(wind_backstepping_sn_single_step, m) {
  m.def("forward_single_step_sn("
        "Tensor w, Tensor q, Tensor k, Tensor v, Tensor a, Tensor b, "
        "Tensor tau, Tensor do_sn, Tensor h0, Tensor(a!) y, Tensor(b!) h1) -> "
        "()");
}

TORCH_LIBRARY_IMPL(wind_backstepping_sn_single_step, CUDA, m) {
  m.impl("forward_single_step_sn", forward_single_step_sn);
}
