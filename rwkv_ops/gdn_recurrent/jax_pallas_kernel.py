...
def _make_gdn_recurrent_fwd_kernel(chunk_size: int):
    """构造 Gated DeltaNet recurrent 训练前向 Pallas kernel。
    ...
    return _gdn_recurrent_fwd_kernel


def _make_gdn_recurrent_bwd_kernel(chunk_size: int):
    """构造 Gated DeltaNet recurrent 训练反向 Pallas kernel。
    ...
    return _gdn_recurrent_bwd_kernel
...
def _gdn_recurrent_fwd_pallas_call(q, k, v, g, beta, h0, chunk_size: int):
    B, N, T, K = q.shape
    return launch(
        f"gdn_recurrent_fwd_{chunk_size}",
        _make_gdn_recurrent_fwd_kernel(chunk_size),
...
def _gdn_recurrent_fwd_warmup(q, k, v, g, beta, h0, chunk_size: int):
    B, N, T, K = q.shape
    ensure_config(
        f"gdn_recurrent_fwd_{chunk_size}",
        _make_gdn_recurrent_fwd_kernel(chunk_size),
...
def _gdn_recurrent_fwd_spmd(q, k, v, g, beta, h0, chunk_size: int):
    return _gdn_recurrent_fwd_pallas_call(q, k, v, g, beta, h0, chunk_size)
...
def _gdn_recurrent_bwd_pallas_call(
    q, k, v, g, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, chkp, h0, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"gdn_recurrent_bwd_{chunk_size}",
        _make_gdn_recurrent_bwd_kernel(chunk_size),
...
def _gdn_recurrent_bwd_spmd(
    ...
    return _gdn_recurrent_bwd_pallas_call(
        q, k, v, g, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, chkp, h0, chunk_size
    )
...
def _gdn_recurrent_train(q, k, v, g, beta, h0, chunk_size: int):
    _gdn_recurrent_fwd_warmup(q, k, v, g, beta, h0, chunk_size)
