// Gated DeltaNet recurrent JAX FFI CUDA 训练/推理 kernel。
...
// Gated DeltaNet recurrent 训练前向 kernel。
//
...
//   g_: [B, N, T], float32, row-major。log-space decay。
...
__global__ __launch_bounds__(kBlockThreads) void gdn_recurrent_fwd_kernel(
...
// Gated DeltaNet recurrent 训练反向 kernel。
//
// 每个 block 处理一个 (batch, head)。从 dht 开始自 T-1 倒序递推到 0；
// 到达 chunk 边界（含序列末尾）时从 checkpoint 重载 S_{t+1}，避免长序列
// 反向递推中反复除以 exp(g) 导致的数值爆炸。
...
//   chkp_: [B, N, T//_CHUNK_LEN_, K, V], float32, row-major。state 快照。
//   dq_, dk_: [B, N, T, K], 与 q 同 dtype, row-major。输出梯度。
//   dv_: [B, N, T, V], float32, row-major。输出梯度。
//   dg_, dbeta_: [B, N, T], float32, row-major。输出梯度。
...
//   _CHUNK_LEN_: chunk 长度，默认 16，T 必须被其整除。
template <typename ET>
__global__ __launch_bounds__(kBlockThreads) void gdn_recurrent_bwd_kernel(
    int T, int H, float scale, const ET *__restrict__ q_,
    const ET *__restrict__ k_, const ET *__restrict__ v_,
    const float *__restrict__ g_, const float *__restrict__ beta_,
...
// Gated DeltaNet recurrent 推理前向 kernel。
//
...
template <typename ET>
__global__ __launch_bounds__(kBlockThreads) void gdn_recurrent_inference_kernel(
    int T, int H, float scale, const ET *__restrict__ q_,
    const ET *__restrict__ k_, const ET *__restrict__ v_,
    const float *__restrict__ g_, const float *__restrict__ beta_,
    const float *__restrict__ h0_, ET *__restrict__ o_,
...
// Gated DeltaNet recurrent 单步 RNN 前向 kernel。
//
...
//   g_, beta_: [B, N], float32, row-major。
...
template <typename ET>
__global__
__launch_bounds__(kBlockThreads) void gdn_recurrent_single_step_kernel(
...
static ffi::Error
GdnFwdHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
           ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> g,
           ffi::Buffer<ffi::F32> beta, ffi::Buffer<ffi::F32> h0,
           ffi::ResultBuffer<DT> o, ffi::ResultBuffer<ffi::F32> kv_mem,
           ffi::ResultBuffer<ffi::F32> chkp, ffi::ResultBuffer<ffi::F32> inv_q,
           ffi::ResultBuffer<ffi::F32> inv_k) {
  auto dims = q.dimensions();
  int B = dims[0], H = dims[1], T = dims[2];
  const float scale = 1.0f / sqrtf((float)_K_);

  gdn_recurrent_fwd_kernel<ET><<<dim3(H, B), kBlockThreads, 0, stream>>>(
...
template <typename ET, ffi::DataType DT>
static ffi::Error
GdnBwdHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
           ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> g,
           ffi::Buffer<ffi::F32> beta, ffi::Buffer<DT> dy,
           ffi::Buffer<ffi::F32> dht, ffi::Buffer<ffi::F32> kv_mem,
           ffi::Buffer<ffi::F32> inv_q, ffi::Buffer<ffi::F32> inv_k,
           ffi::Buffer<ffi::F32> chkp, ffi::ResultBuffer<DT> dq,
           ffi::ResultBuffer<DT> dk, ffi::ResultBuffer<ffi::F32> dv,
           ffi::ResultBuffer<ffi::F32> dg, ffi::ResultBuffer<ffi::F32> dbeta,
           ffi::ResultBuffer<ffi::F32> dh0) {
  auto dims = q.dimensions();
  int B = dims[0], H = dims[1], T = dims[2];
  const float scale = 1.0f / sqrtf((float)_K_);

  gdn_recurrent_bwd_kernel<ET><<<dim3(H, B), kBlockThreads, 0, stream>>>(
      T, H, scale, reinterpret_cast<const ET *>(q.typed_data()),
      reinterpret_cast<const ET *>(k.typed_data()),
      reinterpret_cast<const ET *>(v.typed_data()), g.typed_data(),
      beta.typed_data(), reinterpret_cast<const ET *>(dy.typed_data()),
      dht.typed_data(), kv_mem.typed_data(), inv_q.typed_data(),
      inv_k.typed_data(), chkp.typed_data(),
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("gdn_recurrent_bwd error: ") +
                                cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
template <typename ET, ffi::DataType DT>
static ffi::Error
GdnInferenceHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
                 ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> g,
                 ffi::Buffer<ffi::F32> beta, ffi::Buffer<ffi::F32> h0,
  ...
  gdn_recurrent_inference_kernel<ET><<<dim3(H, B), kBlockThreads, 0, stream>>>(
      T, H, scale, reinterpret_cast<const ET *>(q.typed_data()),
      reinterpret_cast<const ET *>(k.typed_data()),
      reinterpret_cast<const ET *>(v.typed_data()), g.typed_data(),
      beta.typed_data(), h0.typed_data(),
  ...
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("gdn_recurrent_inference error: ") +
                                cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
template <typename ET, ffi::DataType DT>
static ffi::Error
GdnSingleStepHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
                  ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> g,
                  ffi::Buffer<ffi::F32> beta, ffi::Buffer<ffi::F32> h0,
                  ffi::ResultBuffer<DT> o, ffi::ResultBuffer<ffi::F32> ht) {
  auto dims = q.dimensions();
  ...
  gdn_recurrent_single_step_kernel<ET>
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("gdn_recurrent_single_step error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
#define GDN_DEFINE_FWD_HANDLER(SYMBOL, ET, DT)                                 \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnFwdHost<ET, DT>),                  \
                                ffi::Ffi::Bind()                               \
...
#define GDN_DEFINE_BWD_HANDLER(SYMBOL, ET, DT)                                 \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnBwdHost<ET, DT>),                  \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
#define GDN_DEFINE_INF_HANDLER(SYMBOL, ET, DT)                                 \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnInferenceHost<ET, DT>),            \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
#define GDN_DEFINE_SINGLE_HANDLER(SYMBOL, ET, DT)                              \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (GdnSingleStepHost<ET, DT>),           \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
GDN_DEFINE_FWD_HANDLER(GdnRecurrentFwdBf16, bf, ffi::BF16);
GDN_DEFINE_BWD_HANDLER(GdnRecurrentBwdBf16, bf, ffi::BF16);
GDN_DEFINE_INF_HANDLER(GdnRecurrentInferenceBf16, bf, ffi::BF16);
GDN_DEFINE_SINGLE_HANDLER(GdnRecurrentSingleStepBf16, bf, ffi::BF16);

GDN_DEFINE_FWD_HANDLER(GdnRecurrentFwdF32, float, ffi::F32);
GDN_DEFINE_BWD_HANDLER(GdnRecurrentBwdF32, float, ffi::F32);
GDN_DEFINE_INF_HANDLER(GdnRecurrentInferenceF32, float, ffi::F32);
GDN_DEFINE_SINGLE_HANDLER(GdnRecurrentSingleStepF32, float, ffi::F32);
