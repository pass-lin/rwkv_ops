"""JAX 版 Gated DeltaNet recurrent CUDA kernel 封装。"""
...
_FFI_SYMBOLS = {
    "fwd": ("GdnRecurrentFwdBf16", "GdnRecurrentFwdF32"),
    "bwd": ("GdnRecurrentBwdBf16", "GdnRecurrentBwdF32"),
    "inference": ("GdnRecurrentInferenceBf16", "GdnRecurrentInferenceF32"),
    "single_step": ("GdnRecurrentSingleStepBf16", "GdnRecurrentSingleStepF32"),
}
...
def _target_name(kind: str, dtype, K: int, V: int, chunk_size: int) -> str:
    ...
    return f"gdn_recurrent_{kind}_{dtype_tag}_{K}_{V}_{chunk_size}"
...
def _ensure_compiled(K: int, V: int, chunk_size: int) -> pathlib.Path:
    ...
    so_path = build_dir / "gdn_recurrent.so"
    if so_path.exists():
        return so_path

    print(
        f"[gdn_recurrent_jax] First use – compiling CUDA kernel (K={K}, V={V}, chunk_size={chunk_size})…"
    )
    ...
    if not so_path.exists():
        raise RuntimeError("Compilation failed – gdn_recurrent.so not found.")

    print("[gdn_recurrent_jax] Compilation finished – output at", so_path)
    return so_path
...
def _check_dtype(dtype) -> None:
    ...
    if dtype not in (jnp.bfloat16, jnp.float32):
        raise ValueError(
            f"gdn_recurrent CUDA kernel only supports bfloat16/float32, got {dtype}"
        )
...
def _gdn_recurrent_fwd_ffi_call(q, k, v, g, beta, h0, chunk_size: int):
    _, _, _, K = q.shape
    V = v.shape[-1]
...
def _gdn_recurrent_fwd_spmd(q, k, v, g, beta, h0, chunk_size: int):
    return _gdn_recurrent_fwd_ffi_call(q, k, v, g, beta, h0, chunk_size)


_gdn_recurrent_fwd_spmd.def_partition(
    infer_sharding_from_operands=_fwd_infer_sharding,
    sharding_rule=FWD_RULE,
    partition=create_partition(_gdn_recurrent_fwd_ffi_call),
)
...
def _gdn_recurrent_bwd_ffi_call(
    q, k, v, g, beta, dy, dht, kv_mem, inv_q, inv_k, chkp, chunk_size: int
):
    _, _, _, K = q.shape
    V = v.shape[-1]
    return jax.ffi.ffi_call(
        ...
        _bwd_out_shape(q, v),
        vmap_method="broadcast_all",
    )(q, k, v, g, beta, dy, dht, kv_mem, inv_q, inv_k, chkp)
...
def _gdn_recurrent_bwd_spmd(
    ...
    return _gdn_recurrent_bwd_ffi_call(
        q, k, v, g, beta, dy, dht, kv_mem, inv_q, inv_k, chkp, chunk_size
    )
...
_gdn_recurrent_bwd_spmd.def_partition(
    ...
    partition=create_partition(_gdn_recurrent_bwd_ffi_call),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(6,))
def _gdn_recurrent_train(q, k, v, g, beta, h0, chunk_size: int):
    out, kv_mem, state_chkp, inv_q, inv_k = _gdn_recurrent_fwd_spmd(
        q, k, v, g, beta, h0, chunk_size
    )
...
def _gdn_train_fwd(q, k, v, g, beta, h0, chunk_size: int):
    ...
    final_state = state_chkp[:, :, -1, :, :]
    return (out, final_state), (q, k, v, g, beta, kv_mem, inv_q, inv_k, state_chkp)


def _gdn_train_bwd(chunk_size: int, res, grads):
    q, k, v, g, beta, kv_mem, inv_q, inv_k, state_chkp = res
    dy, dht = grads
...
def _gdn_recurrent_inf_ffi_call(q, k, v, g, beta, h0, chunk_size: int):
    _, _, _, K = q.shape
    V = v.shape[-1]
    return jax.ffi.ffi_call(
...
@functools.partial(custom_partitioning, static_argnums=(6,))
def _gdn_recurrent_inf_spmd(q, k, v, g, beta, h0, chunk_size: int):
    return _gdn_recurrent_inf_ffi_call(q, k, v, g, beta, h0, chunk_size)


_gdn_recurrent_inf_spmd.def_partition(
    infer_sharding_from_operands=_inf_infer_sharding,
    sharding_rule=INF_RULE,
    partition=create_partition(_gdn_recurrent_inf_ffi_call),
)
...
def _gdn_recurrent_single_step_ffi_call(q, k, v, g, beta, h0, chunk_size: int):
    _, _, K = q.shape
    V = v.shape[-1]
    return jax.ffi.ffi_call(
...
@functools.partial(custom_partitioning, static_argnums=(6,))
def _gdn_recurrent_single_step_spmd(q, k, v, g, beta, h0, chunk_size: int):
    return _gdn_recurrent_single_step_ffi_call(q, k, v, g, beta, h0, chunk_size)


_gdn_recurrent_single_step_spmd.def_partition(
    infer_sharding_from_operands=_inf_infer_sharding,
    sharding_rule=SINGLE_RULE,
    partition=create_partition(_gdn_recurrent_single_step_ffi_call),
)
...
def gated_delta_net_recurrent_inference(
    ...
    _get_lib(K, V, chunk_size)
    h0 = _prepare_h0(initial_state, B, N, K, V)
    out, final_state = _gdn_recurrent_inf_spmd(q, k, v, g, beta, h0, chunk_size)
...
def gated_delta_net_recurrent_single_step(
    ...
    """Gated DeltaNet recurrent 单步 RNN 算子（JAX CUDA 实现）。
    ...
    _get_lib(K, V, chunk_size)
    h0 = _prepare_h0(initial_state, B, N, K, V)
    out, next_state = _gdn_recurrent_single_step_spmd(q, k, v, g, beta, h0, chunk_size)
    out = jnp.asarray(out, dtype)
