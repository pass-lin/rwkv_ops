// Gated DeltaNet recurrent PyTorch CUDA 训练/推理 kernel。
...
// 倒序递推到 0；到达 chunk 边界时从 checkpoint 重载 S_{t+1}，避免长序列
...
__global__ __launch_bounds__(kBlockThreads) void gdn_recurrent_bwd_kernel(
    ...
    const float *__restrict__ inv_k_, const float *__restrict__ h0_,
    const float *__restrict__ chkp_, float *__restrict__ dq_,
    float *__restrict__ dk_, float *__restrict__ dv_, float *__restrict__ dg_,
...
void cuda_gdn_forward(int B, int T, int H, float scale, const ET *q,
  ...
  gdn_recurrent_fwd_kernel<ET><<<dim3(H, B), kBlockThreads>>>(
      T, H, scale, q, k, v, g, beta, h0, o, kv_mem, chkp, inv_q, inv_k, ht);
}
...
void cuda_gdn_backward(int B, int T, int H, float scale, const ET *q,
                       ...
                       float *dh0) {
  gdn_recurrent_bwd_kernel<ET><<<dim3(H, B), kBlockThreads>>>(
      T, H, scale, q, k, v, g, beta, dout, dht, kv_mem, inv_q, inv_k, h0, chkp,
...
void cuda_gdn_forward_inference(int B, int T, int H, float scale, const ET *q,
                                ...
                                float *ht) {
  gdn_recurrent_inference_kernel<ET>
      <<<dim3(H, B), kBlockThreads>>>(T, H, scale, q, k, v, g, beta, h0, o, ht);
}
...
void cuda_gdn_single_step(int B, int H, float scale, const ET *q, const ET *k,
  ...
  gdn_recurrent_single_step_kernel<ET>
      <<<dim3(H, B), kBlockThreads>>>(H, scale, q, k, v, g, beta, h0, o, ht);
}

// 显式实例化 bfloat16 与 float32 两个版本，供 .cpp 侧按输入 dtype 分发。
template void cuda_gdn_forward<bf>(int, int, int, float, const bf *, const bf *,
                                   const bf *, const float *, const float *,
                                   const float *, bf *, float *, float *,
                                   float *, float *, float *);
template void cuda_gdn_forward<float>(int, int, int, float, const float *,
                                      const float *, const float *,
                                      const float *, const float *,
                                      const float *, float *, float *, float *,
                                      float *, float *, float *);
template void cuda_gdn_backward<bf>(int, int, int, float, const bf *,
                                    const bf *, const bf *, const float *,
                                    const float *, const bf *, const float *,
                                    const float *, const float *, const float *,
                                    const float *, const float *, float *,
                                    float *, float *, float *, float *,
                                    float *);
template void cuda_gdn_backward<float>(
...
template void cuda_gdn_forward_inference<bf>(int, int, int, float, const bf *,
                                             const bf *, const bf *,
                                             const float *, const float *,
                                             const float *, bf *, float *);
template void cuda_gdn_forward_inference<float>(int, int, int, float,
                                                const float *, const float *,
                                                const float *, const float *,
                                                const float *, const float *,
                                                float *, float *);
template void cuda_gdn_single_step<bf>(int, int, float, const bf *, const bf *,
                                       const bf *, const float *, const float *,
                                       const float *, bf *, float *);
template void cuda_gdn_single_step<float>(int, int, float, const float *,
                                          const float *, const float *,
                                          const float *, const float *,
