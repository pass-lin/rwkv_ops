"""mHC Pre-Op JAX -> Triton 桥接（custom_vjp + SPMD）。"""
...
# SPMD sharding rule：b=Batch, t=Time, n=NSize1, m=NSize2, c=Channel。
...
def mhc_pre_op_fwd_kernel_call(x, h_res_in, h_pre_in, num_iters, eps):
    ...
    out_shapes = [
        jax.ShapeDtypeStruct((total_bt, C), x.dtype),
        jax.ShapeDtypeStruct((total_bt, n, n), jnp.float32),
    ]
...
def mhc_pre_op_fused(x, h_res_in, h_pre_in, num_iters=20, eps=1e-8):
    """mHC Pre-Op JAX 公开入口（Triton 加速）。

    通过闭包捕获静态参数 num_iters 与 eps，避免它们进入 custom_vjp 的追踪范围。
