...
mhc_lib = load(
    ...
    sources=[os.path.join(current_dir, "mhc_op.cpp"), os.path.join(current_dir, "mhc_cuda.cu")],
    extra_include_paths=[common_inc, common_ker],
    extra_cuda_cflags=["-O3", "--use_fast_math", "-std=c++17", "-D__CUDA_NO_BFLOAT16_CONVERSIONS__"],
    verbose=True
)
...
class SinkhornKnoppFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        d_inp = mhc_lib.sinkhorn_bwd(grad_output.float().contiguous(), out, x_stabilized, ctx.num_iters, ctx.eps)
...
class RMSNormFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        dx = mhc_lib.rmsnorm_bwd(grad_output.to(torch.bfloat16).contiguous(), inp, ctx.eps)
        return dx, None
...
class StreamAggregateFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_output_fp32 = grad_output.float().contiguous()
        d_inp, d_H_pre = mhc_lib.stream_aggregate_bwd(grad_output_fp32, inp, H_pre, ctx.per_token)
...
def stream_mix(inp, M): return StreamMixFunction.apply(inp, M)
