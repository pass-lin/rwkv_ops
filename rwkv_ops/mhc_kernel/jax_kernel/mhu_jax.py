...
jax.ffi.register_ffi_target("sinkhorn_fwd", jax.ffi.pycapsule(_LIB.SinkhornFwd), platform="CUDA")
jax.ffi.register_ffi_target("sinkhorn_bwd", jax.ffi.pycapsule(_LIB.SinkhornBwd), platform="CUDA")
...
# ---------- 公共API ----------
def sinkhorn_knopp(inp: jnp.ndarray, num_iters: int = 20, eps: float = 1e-8) -> jnp.ndarray:
    """
    ...
    # 使用checkpoint防止重计算
    checkpointed_kernel = jax.checkpoint(kernel, policy=cp.save_anything_except_these_names(()))
