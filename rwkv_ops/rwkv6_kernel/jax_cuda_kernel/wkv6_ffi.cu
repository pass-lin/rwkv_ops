...
template <typename F> __device__ __forceinline__ float to_float(const F &x) {
  return static_cast<float>(x);
}
template <typename F> __device__ __forceinline__ F from_float(float x) {
  return F(x);
}
...
__global__ void kernel_backward_201(
  ...
  __shared__ float v[_N_], gy[_N_];
  float saaaa[_N_] = {0}, sbbbb[_T_ - 2 > 0 ? _T_ - 2 : 1] = {0},
        scccc[_N_] = {0};
...
void HostApplyRWKVWithState(cudaStream_t stream, int B, int T, int C, int H,
                            bool S, const int32_t *state_map,
                            const T_in *input_r, const T_in *input_k,
                            const T_in *input_v, const T_in *input_w,
                            const T_in *input_u, const T_out *input_s,
                            T_out *output_y, T_out *output_s) {
  assert(H * _N_ == C);
  kernel_forward_state<<<dim3(B * H), dim3(_N_), _N_ * 4 * sizeof(float),
                         stream>>>(B, T, C, H, S, state_map, input_r, input_k,
                                   input_v, input_w, input_u, input_s, output_y,
                                   output_s);
}
...
void HostApplyRWKV(cudaStream_t stream, int B, int T, int C, int H,
                   const T_in *input_r, const T_in *input_k,
                   const T_in *input_v, const T_in *input_w,
                   const T_in *input_u, T_out *output_y) {
  assert(H * _N_ == C);
...
void HostApplyGradient(cudaStream_t stream, int B, int T, int C, int H,
                       const T_in *r, const T_in *k, const T_in *v,
                       const T_in *w, const T_in *u, const T_out *gy, T_out *gr,
                       T_out *gk, T_out *gv, T_out *gw, T_out *gu) {
  assert(H * _N_ == C);
  kernel_backward_101<<<dim3(B * H), dim3(_N_), _N_ * 2 * sizeof(float),
                        stream>>>(B, T, C, H, r, k, v, w, u, gy, gr, gu);
  kernel_backward_102<<<dim3(B * H), dim3(_N_), _N_ * 2 * sizeof(float),
                        stream>>>(B, T, C, H, r, k, v, w, u, gy, gk);
  kernel_backward_103<<<dim3(B * H), dim3(_N_), _N_ * 4 * sizeof(float),
                        stream>>>(B, T, C, H, r, k, v, w, u, gy, gv);
  kernel_backward_201<<<dim3(B * H), dim3(_N_), _N_ * 2 * sizeof(float),
                        stream>>>(B, T, C, H, r, k, v, w, u, gy, gw);
}
...
static ffi::Error Wkv6FwdHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> r,
  ...
  int H = C / _N_;
  HostApplyRWKV<bf16, bf16>(stream, B, T, C, H,
...
static ffi::Error
Wkv6BwdHost(cudaStream_t stream, ffi::Buffer<ffi::BF16> r,
            ffi::Buffer<ffi::BF16> k, ffi::Buffer<ffi::BF16> v,
            ffi::Buffer<ffi::BF16> w, ffi::Buffer<ffi::BF16> u,
            ffi::Buffer<ffi::BF16> gy, ffi::ResultBuffer<ffi::BF16> gr,
            ffi::ResultBuffer<ffi::BF16> gk, ffi::ResultBuffer<ffi::BF16> gv,
            ffi::ResultBuffer<ffi::BF16> gw, ffi::ResultBuffer<ffi::BF16> gu) {
  auto dims = r.dimensions();
  int B = dims[0], T = dims[1], C = dims[2];
  int H = C / _N_;
  HostApplyGradient<bf16, bf16>(stream, B, T, C, H,
                                reinterpret_cast<bf16 *>(r.typed_data()),
                                reinterpret_cast<bf16 *>(k.typed_data()),
                                reinterpret_cast<bf16 *>(v.typed_data()),
                                reinterpret_cast<bf16 *>(w.typed_data()),
...
static ffi::Error Wkv6FwdWithStateHost(
    cudaStream_t stream, ffi::Buffer<ffi::BF16> r, ffi::Buffer<ffi::BF16> k,
    ffi::Buffer<ffi::BF16> v, ffi::Buffer<ffi::BF16> w,
    ffi::Buffer<ffi::BF16> u, ffi::Buffer<ffi::S32> state_map,
    ffi::Buffer<ffi::BF16> init_state, ffi::ResultBuffer<ffi::BF16> y,
...
//  FFI 注册
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv6Fwd, Wkv6FwdHost,
                              ffi::Ffi::Bind()
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv6Bwd, Wkv6BwdHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // r
                                  .Arg<ffi::Buffer<ffi::BF16>>() // k
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(Wkv6FwdWithState, Wkv6FwdWithStateHost,
                              ffi::Ffi::Bind()
                                  .Ctx<ffi::PlatformStream<cudaStream_t>>()
                                  .Arg<ffi::Buffer<ffi::BF16>>() // r
                                  .Arg<ffi::Buffer<ffi::BF16>>() // k
