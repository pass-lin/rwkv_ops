"""Gated DeltaNet chunkwise SANE 状态反向扫描 Triton kernel。

在 gdn_chunk chunk_bwd_dhu 基础上，对每个 chunk 边界反向传播 SANE 的
tanh 导数，并输出 dtau。

关键优化：不额外保存 SANE 前状态 h_raw，而是在反向时从 h[i]（SANE 后进入状态）
和 v_new 重算 s_raw = h[i] * exp(g_last) + k^T @ v_new_scaled。
"""
...
def _gdn_chunk_bwd_dhu_sane_kernel(
    ...
    for i_t in range(N - 1, -1, -1):
        ...
        # 重算 SANE 前状态 s_raw = h[i] * exp(g_last) + k^T @ v_new_scaled
        ...
        # s_raw = h[i] * exp(g_last) + k^T @ (v_new * exp(g_last - g))
        b_s_raw = b_h * tl.exp(g_last)
        ...
        for i_k in range(tl.cdiv(K, BK)):
            ...
            b_s_raw += tl.dot(
                tl.trans(b_k).to(b_v_new_scaled.dtype),
                b_v_new_scaled,
                allow_tf32=False,
            )
...
def gdn_chunk_bwd_dhu_sane(
    ...
    if mask is None:
        mask = torch.empty((B, N), dtype=torch.float32, device=q.device)
