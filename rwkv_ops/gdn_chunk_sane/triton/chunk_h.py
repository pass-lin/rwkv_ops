"""Gated DeltaNet chunkwise SANE 状态递推 Triton kernel。

在 gdn_chunk chunk_h 基础上，在每个 chunk 末尾对跨 chunk 传递的 state
执行 State Anomaly Neutralization：
...
@triton.jit
def _gdn_chunk_fwd_h_sane_kernel(
    k_ptr,
    w_ptr,
    u_ptr,
    g_ptr,
    ...
    for i_t in range(N):
        ...
        # 更新状态：h_raw = h * exp(g_last) + k^T @ v_new_scaled
        b_h_raw = b_h * tl.exp(g_last)
        b_h_raw += tl.dot(
...
def gdn_chunk_fwd_h_sane(
    ...
    if mask is None:
        mask = torch.empty((B, N), dtype=torch.float32, device=k.device)
