...
def _delta_net_recurrent_step(t, inputs, q, k, v, beta, scale, B, H, V, DTYPE):
    """DeltaNet 单步递推（供 fori_loop 使用）。
...
def delta_net_recurrent(
    ...
    state, out = ops.fori_loop(
        0,
        T,
        lambda t, inputs: _delta_net_recurrent_step(
            t, inputs, q, k, v, beta, scale, B, H, V, DTYPE
        ),
        [state, out],
    )
...
def delta_net_reference(
    ...
    """DeltaNet 最简 recurrent 黄金参考实现。

    与 `delta_net_recurrent` 数学等价，但用最直接的 Python 列表+
...
def delta_net_recurrent_inference(
    ...
    """DeltaNet recurrent 推理原生封装（无梯度）。

    与 `delta_net_recurrent` 数学等价，仅用于保持 API 一致性。
...
def delta_net_recurrent_single_step(
    ...
    """DeltaNet recurrent 单步 RNN 原生实现。
