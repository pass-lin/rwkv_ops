"""JAX 版 DeltaNet recurrent Pallas kernel 封装。"""
...
def _make_delta_net_recurrent_fwd_kernel(chunk_size: int):
    """构造 DeltaNet recurrent 训练前向 Pallas kernel。

    返回的 kernel 在 grid (B, N) 上运行，每个 program 处理一个 (batch, head)。
    chunk 内静态展开 chunk_size 步，保存每步 kv_mem 与 chunk 级 state 快照供反向使用。
    """

    def _delta_net_recurrent_fwd_kernel(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
    ...
    return _delta_net_recurrent_fwd_kernel


def _make_delta_net_recurrent_bwd_kernel(chunk_size: int):
    """构造 DeltaNet recurrent 训练反向 Pallas kernel。

    返回的 kernel 在 grid (B, N) 上运行，每个 program 处理一个 (batch, head)。
    从 dht 开始反向递推，利用 chunk 级 state 快照避免数值爆炸。
    h0 仅用于保持与 Triton 版一致的操作数顺序与分片规则，kernel 内不读取。
    """

    def _delta_net_recurrent_bwd_kernel(
        q_ref,
        k_ref,
        v_ref,
        beta_ref,
        ...
        def chunk_body(c_rev, dS):
            ...
            for j in range(chunk_size - 1, -1, -1):
                ...
                state_old = state - k_hat[:, None] * delta[None, :]

                # L 对 k_hat 的梯度。
                d_k_hat = jnp.sum(dstate_new * delta[None, :], axis=1) + jnp.sum(
                    state_old * d_kv_mem[None, :], axis=1
                )

                # L 对 S_t 的梯度。
                dstate_old = dstate_new + k_hat[:, None] * d_kv_mem[None, :]

                # 无 decay 门，dS 与 state 均不需乘除 exp(g)。
                dS = dstate_old
                state = state_old
    ...
    return _delta_net_recurrent_bwd_kernel


def _delta_net_recurrent_inf_kernel(
    q_ref,
    k_ref,
    v_ref,
    beta_ref,
    ...
    """DeltaNet recurrent 推理前向 Pallas kernel。

    与训练前向数学一致，但不保存反向所需的中间量；支持任意序列长度 T。
    """
    b = pl.program_id(0)
    h = pl.program_id(1)
    T = q_ref.shape[2]
...
def _delta_net_recurrent_single_step_kernel(
    q_ref,
    k_ref,
    v_ref,
    beta_ref,
    ...
    """DeltaNet recurrent 单步 RNN Pallas kernel。
...
def _fwd_out_shape(r, v, chunk_size: int):
    B, N, T, K = r.shape
    V = v.shape[-1]
    return [
...
def _delta_net_recurrent_fwd_pallas_call(q, k, v, beta, h0, chunk_size: int):
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_fwd_{chunk_size}",
        _make_delta_net_recurrent_fwd_kernel(chunk_size),
...
def _delta_net_recurrent_fwd_warmup(q, k, v, beta, h0, chunk_size: int):
    B, N, T, K = q.shape
    ensure_config(
        f"delta_net_recurrent_fwd_{chunk_size}",
        _make_delta_net_recurrent_fwd_kernel(chunk_size),
...
def _delta_net_recurrent_fwd_spmd(q, k, v, beta, h0, chunk_size: int):
    return _delta_net_recurrent_fwd_pallas_call(q, k, v, beta, h0, chunk_size)


_delta_net_recurrent_fwd_spmd.def_partition(
    infer_sharding_from_operands=_fwd_infer_sharding,
    sharding_rule=FWD_RULE,
    partition=create_partition(_delta_net_recurrent_fwd_pallas_call),
)
...
def _bwd_out_shape(r, v, chunk_size: int):
    B, N, T, K = r.shape
    V = v.shape[-1]
    return [
...
def _delta_net_recurrent_bwd_pallas_call(
    q, k, v, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, h0, chkp, chunk_size: int
):
    B, N, T, K = q.shape
    return launch(
        f"delta_net_recurrent_bwd_{chunk_size}",
        _make_delta_net_recurrent_bwd_kernel(chunk_size),
        _bwd_out_shape(q, v, chunk_size),
        (B, N),
        (q, k, v, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, h0, chkp),
    )


def _delta_net_recurrent_bwd_warmup(
    q, k, v, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, h0, chkp, chunk_size: int
):
    B, N, T, K = q.shape
    ensure_config(
...
def _delta_net_recurrent_bwd_spmd(
    ...
    return _delta_net_recurrent_bwd_pallas_call(
        q, k, v, beta, dy, dht, kv_mem, inv_norm_q, inv_norm_k, h0, chkp, chunk_size
    )
...
_delta_net_recurrent_bwd_spmd.def_partition(
    ...
    partition=create_partition(_delta_net_recurrent_bwd_pallas_call),
)


@functools.partial(jax.custom_vjp, nondiff_argnums=(5,))
def _delta_net_recurrent_train(q, k, v, beta, h0, chunk_size: int):
    _delta_net_recurrent_fwd_warmup(q, k, v, beta, h0, chunk_size)
    out, kv_mem, state_chkp, inv_norm_q, inv_norm_k = _delta_net_recurrent_fwd_spmd(
...
def _dn_train_bwd(chunk_size: int, res, grads):
    ...
    _delta_net_recurrent_bwd_warmup(
        q,
        k,
        v,
        beta,
...
def _delta_net_recurrent_inf_pallas_call(q, k, v, beta, h0):
    B, N, T, K = q.shape
    return launch(
        "delta_net_recurrent_inf",
        _delta_net_recurrent_inf_kernel,
        _inf_out_shape(q, v),
        (B, N),
        (q, k, v, beta, h0),
    )
...
def _delta_net_recurrent_inf_warmup(q, k, v, beta, h0):
    B, N, T, K = q.shape
    ensure_config(
        "delta_net_recurrent_inf",
        _delta_net_recurrent_inf_kernel,
...
@custom_partitioning
def _delta_net_recurrent_inf_spmd(q, k, v, beta, h0):
    return _delta_net_recurrent_inf_pallas_call(q, k, v, beta, h0)


_delta_net_recurrent_inf_spmd.def_partition(
    infer_sharding_from_operands=_inf_infer_sharding,
    sharding_rule=INF_RULE,
    partition=create_partition(_delta_net_recurrent_inf_pallas_call),
)
...
def _delta_net_recurrent_single_step_pallas_call(q, k, v, beta, h0):
    B, N, K = q.shape
    return launch(
        "delta_net_recurrent_single_step",
        _delta_net_recurrent_single_step_kernel,
...
def _delta_net_recurrent_single_step_warmup(q, k, v, beta, h0):
    B, N, K = q.shape
    ensure_config(
        "delta_net_recurrent_single_step",
        _delta_net_recurrent_single_step_kernel,
...
@custom_partitioning
def _delta_net_recurrent_single_step_spmd(q, k, v, beta, h0):
    return _delta_net_recurrent_single_step_pallas_call(q, k, v, beta, h0)


_delta_net_recurrent_single_step_spmd.def_partition(
    infer_sharding_from_operands=_inf_infer_sharding,
    sharding_rule=SINGLE_RULE,
    partition=create_partition(_delta_net_recurrent_single_step_pallas_call),
)
...
def delta_net_recurrent(
    ...
    """DeltaNet recurrent 训练算子（JAX Pallas 实现）。
    ...
    if T % chunk_size != 0:
        raise ValueError(
            f"Pallas kernel requires sequence length T={T} to be divisible by chunk_size={chunk_size}"
        )
...
def delta_net_recurrent_inference(
    ...
    """DeltaNet recurrent 推理算子（JAX Pallas 实现，无梯度）。
    ...
    h0 = _prepare_h0(initial_state, B, N, K, V)
    _delta_net_recurrent_inf_warmup(q, k, v, beta, h0)
    out, final_state = _delta_net_recurrent_inf_spmd(q, k, v, beta, h0)
...
def delta_net_recurrent_single_step(
    ...
    """DeltaNet recurrent 单步 RNN 算子（JAX Pallas 实现）。
    ...
    _delta_net_recurrent_single_step_warmup(q, k, v, beta, h0)
    out, next_state = _delta_net_recurrent_single_step_spmd(q, k, v, beta, h0)
    out = jnp.asarray(out, dtype)
