"""DeltaNet recurrent Triton kernel（共享实现）。"""
...
@triton.jit
def delta_net_recurrent_fwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent 训练前向 Triton kernel。
...
def _bwd_autotune_pre_hook(nargs, reset_only=False):
    ...
    for name in ("dq", "dk", "dv", "dbeta", "dh0"):
        buf = nargs.get(name)
        if buf is None:
            continue
...
@triton.jit
def delta_net_recurrent_bwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent 训练反向 Triton kernel。

    每个 program 处理一个 (batch, head) 与 V 方向的一个 block。
    先重新跑一遍前向得到最终状态，再从 T 反向递推到 0。
    q/k/beta 梯度在 V 方向需要跨 block 归约，NV>1 时使用 atomic_add。

    Args:
        q, k: [B, H, T, K]，输入同 dtype，row-major。
        v, do: [B, H, T, V]，输入/输出梯度同 dtype，row-major。
        beta: [B, H, T]，row-major。
    ...
    for t in range(T - 1, -1, -1):
        ...
        # 无 decay 门，state_old 与 carry_dS 均不需乘除 exp(g)。
...
@triton.jit
def delta_net_recurrent_inference_fwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent 推理前向 Triton kernel。
...
@triton.jit
def delta_net_recurrent_single_step_fwd_kernel(
    q,
    k,
    v,
    beta,
    ...
    """DeltaNet recurrent 单步 RNN 前向 Triton kernel。
