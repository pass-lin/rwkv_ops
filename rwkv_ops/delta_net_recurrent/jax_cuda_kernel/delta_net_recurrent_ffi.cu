// DeltaNet recurrent JAX FFI CUDA 训练/推理 kernel。
...
// 到达 chunk 边界（含序列末尾）时从 checkpoint 重载 S_{t+1}，抑制长序列
// 反向递推的浮点误差累积。
...
__global__ __launch_bounds__(kBlockThreads) void delta_net_recurrent_bwd_kernel(
    ...
    const float *__restrict__ chkp_, ET *__restrict__ dq_, ET *__restrict__ dk_,
...
#pragma unroll
    ...
    if (v < _K_) {
      dq_[qk_base + v] =
          from_float<ET>(iq * (sh_dqhat[v] - sh_q[v] * iq * sh_scalar[3]));
      dk_[qk_base + v] =
          from_float<ET>(ik * (sh_dkhat[v] - sh_khat[v] * sh_scalar[4]));
    }
    if (v == 0) {
...
static ffi::Error DeltaNetFwdHost(
    cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
    ffi::Buffer<DT> v, ffi::Buffer<ffi::F32> beta, ffi::Buffer<ffi::F32> h0,
  ...
  delta_net_recurrent_fwd_kernel<ET><<<dim3(H, B), kBlockThreads, 0, stream>>>(
      ...
      kv_mem->typed_data(), chkp->typed_data(), inv_q->typed_data(),
      inv_k->typed_data());

  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    return ffi::Error::Internal(std::string("delta_net_recurrent_fwd error: ") +
                                cudaGetErrorString(err));
  return ffi::Error::Success();
}

template <typename ET, ffi::DataType DT>
static ffi::Error DeltaNetBwdHost(
    cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
...
template <typename ET, ffi::DataType DT>
static ffi::Error
DeltaNetInferenceHost(cudaStream_t stream, ffi::Buffer<DT> q, ffi::Buffer<DT> k,
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("delta_net_recurrent_inference error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}

template <typename ET, ffi::DataType DT>
static ffi::Error DeltaNetSingleStepHost(cudaStream_t stream, ffi::Buffer<DT> q,
                                         ffi::Buffer<DT> k, ffi::Buffer<DT> v,
                                         ffi::Buffer<ffi::F32> beta,
                                         ffi::Buffer<ffi::F32> h0,
                                         ffi::ResultBuffer<DT> o,
                                         ffi::ResultBuffer<ffi::F32> ht) {
  auto dims = q.dimensions();
  ...
  if (err != cudaSuccess)
    return ffi::Error::Internal(
        std::string("delta_net_recurrent_single_step error: ") +
        cudaGetErrorString(err));
  return ffi::Error::Success();
}
...
#define DN_DEFINE_FWD_HANDLER(SYMBOL, ET, DT)                                  \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (DeltaNetFwdHost<ET, DT>),             \
                                ffi::Ffi::Bind()                               \
...
#define DN_DEFINE_BWD_HANDLER(SYMBOL, ET, DT)                                  \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (DeltaNetBwdHost<ET, DT>),             \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
#define DN_DEFINE_INF_HANDLER(SYMBOL, ET, DT)                                  \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (DeltaNetInferenceHost<ET, DT>),       \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
#define DN_DEFINE_SINGLE_HANDLER(SYMBOL, ET, DT)                               \
  XLA_FFI_DEFINE_HANDLER_SYMBOL(SYMBOL, (DeltaNetSingleStepHost<ET, DT>),      \
                                ffi::Ffi::Bind()                               \
                                    .Ctx<ffi::PlatformStream<cudaStream_t>>()  \
                                    .Arg<ffi::Buffer<DT>>()       /* q */      \
                                    .Arg<ffi::Buffer<DT>>()       /* k */      \
...
DN_DEFINE_FWD_HANDLER(DeltaNetRecurrentFwdBf16, bf, ffi::BF16);
DN_DEFINE_BWD_HANDLER(DeltaNetRecurrentBwdBf16, bf, ffi::BF16);
DN_DEFINE_INF_HANDLER(DeltaNetRecurrentInferenceBf16, bf, ffi::BF16);
DN_DEFINE_SINGLE_HANDLER(DeltaNetRecurrentSingleStepBf16, bf, ffi::BF16);

DN_DEFINE_FWD_HANDLER(DeltaNetRecurrentFwdF32, float, ffi::F32);
DN_DEFINE_BWD_HANDLER(DeltaNetRecurrentBwdF32, float, ffi::F32);
DN_DEFINE_INF_HANDLER(DeltaNetRecurrentInferenceF32, float, ffi::F32);
DN_DEFINE_SINGLE_HANDLER(DeltaNetRecurrentSingleStepF32, float, ffi::F32);
