"""PyTorch 版 DeltaNet recurrent Triton kernel 封装。"""
...
def _make_delta_net_recurrent_triton_function(chunk_size):
    """按 chunk_size 构造 DeltaNet recurrent 训练 Triton 封装类。"""

    class DeltaNetRecurrentTritonFunction(torch.autograd.Function):
        """DeltaNet recurrent 训练前向 Triton 封装。"""
        ...
        def forward(
            ...
            delta_net_recurrent_fwd_kernel[grid](
                q=q,
                k=k,
                v=v,
                beta=beta,
            ...
            ctx.save_for_backward(
                q, k, v, beta, h0, kv_mem_out, state_chkp, inv_norm_q, inv_norm_k
            )
        ...
        def backward(ctx, do, dht):
            ...
            delta_net_recurrent_bwd_kernel[grid](
                q=q,
                k=k,
                v=v,
                beta=beta,
            ...
            return dq, dk, dv, dbeta, dh0, None, None

    return DeltaNetRecurrentTritonFunction


def _make_delta_net_recurrent_inference_triton_function(chunk_size):
    """按 chunk_size 构造 DeltaNet recurrent 推理 Triton 封装类。"""

    class DeltaNetRecurrentInferenceTritonFunction(torch.autograd.Function):
        """DeltaNet recurrent 推理前向 Triton 封装（无反向）。"""
        ...
        def forward(
            ...
            delta_net_recurrent_inference_fwd_kernel[grid](
                q=q,
                k=k,
                v=v,
                beta=beta,
    ...
    return DeltaNetRecurrentInferenceTritonFunction


class DeltaNetRecurrentSingleStepTritonFunction(torch.autograd.Function):
    """DeltaNet recurrent 单步 RNN 前向 Triton 封装（无反向）。"""
    ...
    def forward(
        ...
        delta_net_recurrent_single_step_fwd_kernel[grid](
            q=q,
            k=k,
            v=v,
            beta=beta,
...
def delta_net_recurrent(
    ...
    """DeltaNet recurrent 训练算子（PyTorch Triton 实现）。
    ...
    if q.device.type != "cuda":
        raise RuntimeError("DeltaNet Triton kernel only supports CUDA devices.")

    Op = _make_delta_net_recurrent_triton_function(chunk_size)
    return Op.apply(
        q,
        k,
        v,
...
def delta_net_recurrent_inference(
    ...
    """DeltaNet recurrent 推理算子（PyTorch Triton 实现，无梯度）。
    ...
    Op = _make_delta_net_recurrent_inference_triton_function(chunk_size)
    return Op.apply(
        q,
        k,
        v,
...
def delta_net_recurrent_single_step(
    ...
    """DeltaNet recurrent 单步 RNN 算子（PyTorch Triton 实现）。
    ...
    return DeltaNetRecurrentSingleStepTritonFunction.apply(
        q,
        k,
        v,
        beta,
